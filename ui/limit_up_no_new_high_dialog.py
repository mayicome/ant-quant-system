...
"""
涨停无新高股票筛选对话框
筛选最近10个交易日内有涨停，且涨停后最高价未超过涨停价的股票
"""
        ...
        try:
            ...
            for idx, row in recent_data_sorted.iterrows():
                ...
                if is_limit_up:
                    ...
                    # 如果涨停后的最高价超过涨停价，不满足条件，直接返回None（不再往后看）
                    # 允许0.01的误差，因为价格可能有微小的浮动
                    if max_high_after > limit_up_price + 0.01:
                        return None
...
class LimitUpNoNewHighDialog(QDialog):
    """涨停无新高股票筛选对话框"""
    
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("10日内涨停无新高股票筛选")
    ...
    def setup_ui(self):
        ...
        # 说明标签
        info_label = QLabel("筛选条件：最近10个交易日内只有一个涨停板，该涨停板必须是这10个交易日的最高价（涨停板之前的交易日最高价低于涨停板价，涨停板之后的交易日最高价不高于涨停板价），且涨停板价格不小于布林线上轨")
    ...
    def save_results(self):
        ...
        try:
            ...
            file_path, _ = QFileDialog.getSaveFileName(
                self,
                "保存结果",
                f"涨停无新高股票_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                "CSV文件 (*.csv);;所有文件 (*.*)"
            )

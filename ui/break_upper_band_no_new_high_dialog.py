...
"""
突破上轨无新高股票筛选对话框
筛选最近10个交易日内突破布林线上轨，且突破后最高价未超过突破日最高价的股票
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(_LOGS_DIR, 'break_upper_band_filter.log'), encoding='utf-8'),
...
class BreakUpperBandNoNewHighFilterThread(QThread):
    """突破上轨无新高筛选线程"""
    
    # 信号：更新进度
    progress_updated = pyqtSignal(int, int, str)  # (当前数量, 总数, 当前股票代码)
    # 信号：找到符合条件的股票
    stock_found = pyqtSignal(str, str, str, str, str)  # (股票代码, 股票名称, 突破日期, 突破日最高价, 上轨值)
    # 信号：筛选完成
    ...
    def _get_daily_data(self, stock_code: str) -> Optional[pd.DataFrame]:
        """获取日线数据（需要足够的历史数据以计算技术指标）"""
        try:
            full_stock_code = self._get_full_stock_code(stock_code)
            
            # 计算日期范围（往前推更多天数以确保获取到足够的历史数据）
            # 需要至少20个交易日来计算MA20和布林线，再加上10个交易日用于筛选
            # 总共至少需要30个交易日，往前推60天应该足够
            end_date = self.trading_dates[-1] if self.trading_dates else date.today()
            start_date = end_date - timedelta(days=60)  # 往前推60天，确保包含足够的历史数据
    ...
    def _check_stock(self, stock_code: str, stock_name: str) -> Optional[Tuple[str, str, str]]:
        """检查股票是否符合条件"""
        try:
            # 获取日线数据（包含足够的历史数据）
            ...
            recent_data = daily_data[daily_data['date'].isin(trading_dates_set)].copy()
            ...
            for idx, row in recent_data.iterrows():
                ...
                # 检查技术指标是否有效（不能是NaN）
                if pd.isna(boll_upper) or pd.isna(ma5) or pd.isna(ma10):
                    continue
                ...
                prev_boll_upper = prev_row['BOLL_UPPER']
                
                # 检查前一个交易日的上轨是否有效
                if pd.isna(prev_boll_upper):
                    continue
                
                # 条件1：前一天的最高价在上轨以下
                if prev_high >= prev_boll_upper:
                    continue
                
                # 条件2：该日最高价在上轨以上
                if high_price <= boll_upper:
                    continue
                
                # 条件3：该日的收盘价在MA5和MA10以上
                if close_price <= ma5 or close_price <= ma10:
                    continue
                
                # 条件4：该日之后没有哪个交易日的最高价高于该日的最高价
                # 获取突破日期之后的所有数据（包括最近10个交易日之后的数据）
                after_break_data = daily_data[daily_data['date'] > trade_date]
                
                # 如果突破后没有数据，跳过（不应该出现，因为已经排除了最后一个交易日）
                if after_break_data.empty:
                    continue
                
                # 检查突破后的最高价
                max_high_after = after_break_data['high'].max()
                
                # 如果突破后的最高价没有超过突破日的最高价，符合条件
                # 允许0.01的误差，因为价格可能有微小的浮动
                if max_high_after <= high_price + 0.01:
                    return (trade_date.strftime('%Y-%m-%d'), f"{high_price:.2f}", f"{boll_upper:.2f}")
    ...
    def run(self):
        ...
        try:
            ...
            for idx, (stock_code, stock_name) in enumerate(self.stock_list):
                ...
                if result:
                    break_date, break_high, boll_upper = result
                    self.stock_found.emit(stock_code, stock_name, break_date, break_high, boll_upper)
                    found_count += 1
                    # 每找到10只股票，发送一次调试信息
                    if found_count % 10 == 0:
...
class BreakUpperBandNoNewHighDialog(QDialog):
    """突破上轨无新高股票筛选对话框"""
    
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("10日内突破上轨无新高股票筛选")
    ...
    def setup_ui(self):
        ...
        # 说明标签
        info_label = QLabel("筛选条件：最近10个交易日内，前一天最高价在上轨以下，该日最高价在上轨以上，且该日收盘价在MA5和MA10以上，且该日之后没有哪个交易日的最高价高于该日的最高价")
        ...
        self.result_table.setHorizontalHeaderLabels(["股票代码", "股票名称", "突破日期", "突破日最高价", "上轨值"])
    ...
    def start_filter(self):
        ...
        try:
            ...
            # 创建筛选线程
            self.filter_thread = BreakUpperBandNoNewHighFilterThread(stock_list, trading_dates, self)
    ...
    def on_stock_found(self, stock_code: str, stock_name: str, break_date: str, break_high: str, boll_upper: str):
        """找到符合条件的股票"""
        # 添加到结果列表
        ...
        self.result_table.setItem(row, 2, QTableWidgetItem(break_date))
        self.result_table.setItem(row, 3, QTableWidgetItem(break_high))
        self.result_table.setItem(row, 4, QTableWidgetItem(boll_upper))
        
        # 滚动到最新添加的行，确保用户能看到
        self.result_table.scrollToItem(self.result_table.item(row, 0))
        
        # 更新状态标签显示最新找到的股票
        self.status_label.setText(f"找到符合条件的股票: {stock_code} {stock_name} (突破日期: {break_date}, 最高价: {break_high}, 上轨: {boll_upper})")
    ...
    def save_results(self):
        ...
        try:
            ...
            file_path, _ = QFileDialog.getSaveFileName(
                self,
                "保存结果",
                f"突破上轨无新高股票_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                "CSV文件 (*.csv);;所有文件 (*.*)"
            )

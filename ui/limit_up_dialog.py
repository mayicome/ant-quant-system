...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(_LOGS_DIR, 'limit_up_filter.log'), encoding='utf-8'),
...
class LimitUpFilterThread(QThread):
    """涨停筛选线程"""
    ...
    def _get_daily_data(self, stock_code: str) -> Optional[pd.DataFrame]:
        ...
        注意：返回所有获取的数据（约60天），用于计算布林线等技术指标。
        """
        try:
    ...
    def _check_stock(self, stock_code: str, stock_name: str) -> Optional[Tuple[str, str]]:
        """检查股票是否符合条件（最近10个交易日内有涨停，且满足布林线和涨停前最高价条件）"""
        try:
            # 获取日线数据（包含前一个交易日）
            # 注意：会返回所有获取的数据（约60天），用于计算布林线等技术指标
            ...
            for idx, row in recent_data_sorted.iterrows():
                ...
                if is_limit_up:
                    ...
                    # 比例是涨停板的40%：10%涨停不能高出4%，20%涨停不能高出8%，5%涨停不能高出2%
                    after_limit_data = recent_data_sorted[recent_data_sorted['date'] > trade_date]
                    if not after_limit_data.empty:
                        # 计算允许的最大价格：涨停板价格 * (1 + 涨停板涨幅 * 0.4)
                        # 例如：10%涨停，允许最高价 = 涨停价 * 1.04（即不能高出4%）
                        max_allowed_price = limit_up_price * (1 + expected_increase_ratio * 0.4)
                        # 检查涨停板之后的所有交易日的最高价
...
class LimitUpDialog(QDialog):
    """涨停股票筛选对话框"""
    
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("10日内涨停股票筛选")
    ...
    def setup_ui(self):
        ...
        # 说明标签
        info_label = QLabel("筛选条件：最近10个交易日内有涨停的股票，且满足：\n1. 涨停板价格 >= 布林线上轨\n2. 涨停前最高价 < 涨停价\n3. 涨停后最高价不能超过涨停板价格的(1+涨停板涨幅×40%)")
    ...
    def start_filter(self):
        ...
        try:
            ...
            # 创建筛选线程
            self.filter_thread = LimitUpFilterThread(stock_list, trading_dates, self)
    ...
    def save_results(self):
        ...
        try:
            ...
            file_path, _ = QFileDialog.getSaveFileName(
                self,
                "保存结果",
                f"涨停股票_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                "CSV文件 (*.csv);;所有文件 (*.*)"
            )

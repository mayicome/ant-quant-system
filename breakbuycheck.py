...
# -*- coding: utf-8 -*-
"""
突破买入核验：读取「各日选股收益汇总」xlsx，按行核验真突破详情。

输入 xlsx 须含列：选股日、代码、买入时间（可含其它汇总列，原样保留）。
买入日 = 选股日的下一交易日；tick 优先读 data/ticks/{YYYYMMDD}/{代码}.pkl，否则 QMT 拉取。
不计算压力位锚点表；以文件中的「买入时间」作为突破时刻，在该 tick 上计算突破详情（真突破三条等）。
输出 = 原表各列 + 突破详情列，写入新 xlsx。
"""
...
def _load_or_fetch_tick_df(stock_code_6: str, trade_date_yyyymmdd: str, save_cache: bool = True):
    """先读 data/ticks/{date}/{code}.pkl；无则 QMT；拉取成功后写入 pkl。"""
    _ = save_cache  # 现统一为拉取成功即落盘
    try:
        from utils.tick_data_cache import load_tick_data

        return load_tick_data(stock_code_6, trade_date_yyyymmdd, use_memory_cache=True)
    except Exception:
        pass
    code = _zfill_6(stock_code_6)
    if not code or not trade_date_yyyymmdd or xtdata is None:
        return None
    ...
    if tick_df is not None:
        try:
            from utils.tick_data_cache import normalize_tick_dataframe, write_tick_cache

            norm = normalize_tick_dataframe(tick_df)
            if norm is not None and len(norm) > 0:
                write_tick_cache(code, trade_date_yyyymmdd, norm)
                return norm
        except Exception:
            pass
    return tick_df
...
# 真突破 = ① AND (② OR ③)
_TRUE_BREAKTHROUGH_MIN_BREAK_VS_AVG = 1.25  # ①：突破量/突破前均量 > 本值
_TRUE_BREAKTHROUGH_MAX_ASK_OVER_BID = 0.6  # ②：委卖/委买(取小) < 本值
_TRUE_BREAKTHROUGH_ASK_BID_RATIO_TICK_WINDOW = 5  # ②：近5tick均值为候选之一
_TRUE_BREAKTHROUGH_ASK_BID_RATIO_TICK_WINDOW_3 = 3  # ②：近3tick均值为候选之一
_TRUE_BREAKTHROUGH_MIN_BREAK_VS_PREV_ASK1 = 2.0  # ③：突破量/前tick被吃卖档合计 > 本值


def _depth_ask_over_bid_ratio_for_row(row: Any) -> Optional[float]:
    """单 tick 五档委卖/委买比；委买为 0 时返回 None。"""
...
def _avg_depth_ask_over_bid_ratio_last_n_ticks(
    timed_rows: List[Tuple[float, Any]],
    idx: int,
    n: int = _TRUE_BREAKTHROUGH_ASK_BID_RATIO_TICK_WINDOW,
) -> Tuple[Optional[float], int]:
    """含 idx 在内向前最多 n 个 tick 的委卖/委买比算术均值；无效 tick 跳过。"""
    if not timed_rows or idx < 0 or idx >= len(timed_rows):
        return None, 0
    win = max(1, int(n or 1))
    start = max(0, int(idx) - win + 1)
    ratios: List[float] = []
    for j in range(start, int(idx) + 1):
        r = _depth_ask_over_bid_ratio_for_row(timed_rows[j][1])
        if r is not None:
...
def _min_cond2_depth_ask_over_bid_ratio(
    timed_rows: List[Tuple[float, Any]],
    idx: int,
) -> Tuple[Optional[float], int]:
    """
    条件②用委卖/委买比：本 tick、近3tick均值、近5tick均值 三者取最小（越低卖压越轻）。
    某一档无有效比值则跳过该档。
    """
    if not timed_rows or idx < 0 or idx >= len(timed_rows):
        return None, 0
    candidates: List[float] = []
    r0 = _depth_ask_over_bid_ratio_for_row(timed_rows[idx][1])
    if r0 is not None:
        candidates.append(float(r0))
    r3, _ = _avg_depth_ask_over_bid_ratio_last_n_ticks(
        timed_rows, idx, _TRUE_BREAKTHROUGH_ASK_BID_RATIO_TICK_WINDOW_3
    )
    if r3 is not None:
        candidates.append(float(r3))
    r5, _ = _avg_depth_ask_over_bid_ratio_last_n_ticks(
        timed_rows, idx, _TRUE_BREAKTHROUGH_ASK_BID_RATIO_TICK_WINDOW
    )
    if r5 is not None:
...
def _prev_row_asks_sorted_merged_asc(row: Any, vol_mul: float) -> List[Tuple[float, float]]:
    """前 tick 卖档按价格升序（卖一→卖五），同价合并量；量已为股口径（与 vol_mul 一致）。"""
...
def _break_tick_eaten_prev_ask_depth_sum_shares(
    stock_code: str,
    prev_row: Any,
    break_row: Any,
    vol_mul: float,
    ...
    """
    本突破 tick 相对「前一 tick」盘口：自卖一价起连续被成交价扫到的档数 N，
    及卖一..卖 N 挂单量之和（股）。成交价缺省用本 tick lastPrice；比较价用展示舍入。
    若成交价低于前 tick 卖一（舍入后），返回 (0, None)。
    """
    ...
    if trade_px is None or float(trade_px) <= 0:
        trade_px = _tick_scalar_to_float(break_row.get("lastPrice"))
    if trade_px is None or float(trade_px) <= 0:
        return None, None
    code = (stock_code or "").strip()
    r_trade = _round_price_like_display(code, float(trade_px))
    total = 0.0
    n = 0
    for px, sh in merged:
        r_px = _round_price_like_display(code, float(px))
...
def _max_cond1_breakthrough_volume_sh(
    per_vols: List[Optional[float]], idx: int
) -> Optional[float]:
    """
    条件①用突破量：本 tick 量、本 tick+前1帧均值、本 tick+前2帧均值 三者取最大。
    某一档缺有效成交量则跳过该档；均量分母仍由调用方按原规则计算。
    """
...
def _true_breakthrough_pass(cond1: bool, cond2: bool, cond3: bool) -> bool:
    """真突破：①突破量/均量>1.25，且（②委卖委买取小<0.6 或 ③突破量/被吃卖档合计>2）。"""
...
def _true_breakthrough_remark_text(
    ...
    cond2: bool,
    cond3: bool,
    v_break_cond1: Optional[float],
    v_break_sh: Optional[float],
    avg_shares: Optional[float],
    ask_bid_ratio_cond2: Optional[float],
    ask_bid_ratio_n: int,
    eaten_n: Optional[int],
    prev_eaten_depth_shares: Optional[float],
) -> str:
    """真突破为 False 时列出未满足条件及实测比值；全满足时为空字符串。"""
    if _true_breakthrough_pass(cond1, cond2, cond3):
        return ""
    parts: List[str] = []
    if not cond1:
        if (
            ...
            r = v_break_cond1 / avg_shares
            parts.append(
                f"①突破量(单tick/近2帧均/近3帧均取大)为突破前均量的{r:.2f}倍（需>{_TRUE_BREAKTHROUGH_MIN_BREAK_VS_AVG}）"
            )
        else:
            parts.append(
                "①缺少有效突破量或突破前均量，无法满足>突破前均量×"
                f"{_TRUE_BREAKTHROUGH_MIN_BREAK_VS_AVG}"
            )
    if not cond2:
        if ask_bid_ratio_cond2 is None or ask_bid_ratio_n <= 0:
            parts.append(
                f"②无有效委卖/委买比（本tick/近3tick均/近5tick均取小，需<{_TRUE_BREAKTHROUGH_MAX_ASK_OVER_BID}）"
            )
        else:
            parts.append(
                f"②委卖/委买(取小)={ask_bid_ratio_cond2:.2f}（需<{_TRUE_BREAKTHROUGH_MAX_ASK_OVER_BID}）"
            )
    if not cond3:
        if (
            v_break_sh is not None
            and prev_eaten_depth_shares is not None
            and prev_eaten_depth_shares > 1e-12
            and eaten_n is not None
            and eaten_n > 0
        ):
            r3 = v_break_sh / prev_eaten_depth_shares
            parts.append(
                f"③突破tick成交量为前tick卖1~卖{eaten_n}档合计的{r3:.2f}倍（需>"
                f"{_TRUE_BREAKTHROUGH_MIN_BREAK_VS_PREV_ASK1}）"
            )
        else:
            parts.append(
                "③缺少本笔成交量或前tick被吃卖档量合计（成交价未触及卖一或无卖档等），无法满足>合计×"
                f"{_TRUE_BREAKTHROUGH_MIN_BREAK_VS_PREV_ASK1}"
            )
    if cond1 and (not cond2) and (not cond3):
        parts.append("①已满足，但②与③均未满足（需②<0.6 或 ③>2.0×，满足其一即可）")
    return "；".join(parts)
...
def _breakthrough_events_first_per_effective_regime(
    ...
    委买/委卖、前 tick 卖一价/量（展示参考）；被吃档：以前一 tick 卖档价升序合并同价后，
    用本 tick 成交价（缺省 lastPrice，舍入与压力位一致）自卖一起连续扫到的档数 N，
    ③分母为卖一..卖 N 量之和（股，与逐笔量同口径）。
    真突破：①突破量(本tick/近2帧均/近3帧均取大)/突破前均量>1.25；且（②委卖/委买(本tick/近3tick均/近5tick均取小)<0.6 或 ③本tick量/前tick被吃卖档合计>2）。
    ③本 tick 成交量(股)>前 tick 被吃卖 1~N 档量合计(股)×_TRUE_BREAKTHROUGH_MIN_BREAK_VS_PREV_ASK1（默认 2.0）。
    ...
    for idx, (tsec, row) in enumerate(timed_rows):
        ...
        v_break_cond1 = _max_cond1_breakthrough_volume_sh(per_vols, idx)
        avg_shares: Optional[float] = (
            float(ps) / float(pc) if pc > 0 else None
        )
        eaten_n: Optional[int] = None
        eaten_sum_sh: Optional[float] = None
        if idx > 0:
            eaten_n, eaten_sum_sh = _break_tick_eaten_prev_ask_depth_sum_shares(
                code, timed_rows[idx - 1][1], row, vol_mul
            )

        ask_bid_ratio_cond2, ask_bid_ratio_n = _min_cond2_depth_ask_over_bid_ratio(
            timed_rows, idx
        )
        cond1 = (
            v_break_cond1 is not None
            and avg_shares is not None
            and float(v_break_cond1)
            > float(avg_shares) * float(_TRUE_BREAKTHROUGH_MIN_BREAK_VS_AVG)
        )
        cond2 = (
            ask_bid_ratio_cond2 is not None
            and ask_bid_ratio_cond2
            < float(_TRUE_BREAKTHROUGH_MAX_ASK_OVER_BID)
        )
        cond3 = (
            v_break_sh is not None
            and eaten_sum_sh is not None
            and float(eaten_sum_sh) > 1e-12
            and float(v_break_sh)
            > float(eaten_sum_sh) * float(_TRUE_BREAKTHROUGH_MIN_BREAK_VS_PREV_ASK1)
        )
        true_break = _true_breakthrough_pass(cond1, cond2, cond3)
        ...
        def _fmt_ratio2(x: Optional[float]) -> str:
            if x is None:
                return ""
            try:
                return f"{float(x):.2f}"
        ...
        if (
            v_break_cond1 is not None
            and avg_shares is not None
            and float(avg_shares) > 1e-9
        ):
            mul_vs_avg_txt = _fmt_ratio2(float(v_break_cond1) / float(avg_shares))

        ask_bid_ratio_txt = ""
        if ask_bid_ratio_cond2 is not None:
            ask_bid_ratio_txt = _fmt_ratio2(float(ask_bid_ratio_cond2))

        mul_vs_prev_eaten_txt = ""
        if (
            v_break_sh is not None
            and eaten_sum_sh is not None
            and float(eaten_sum_sh) > 1e-9
        ):
            mul_vs_prev_eaten_txt = _fmt_ratio2(
                float(v_break_sh) / float(eaten_sum_sh)
            )

        eaten_n_txt = ""
        if eaten_n is not None and eaten_n > 0:
            eaten_n_txt = str(int(eaten_n))

        eaten_vol_sum_txt = ""
        if (
            eaten_sum_sh is not None
            and eaten_n is not None
            and eaten_n > 0
        ):
            eaten_vol_sum_txt = _breakthrough_vol_display_in_lots(
                eaten_sum_sh, vol_mul
            )
        ...
        events.append(
            {
                ...
                "prev_tick_eaten_ask_n": eaten_n_txt,
                "prev_tick_eaten_asks_vol_sum": eaten_vol_sum_txt,
                "break_tick_trade_price": trade_px_txt,
                "break_tick_trade_vol": break_vol,
                "break_vol_over_avg_mul": mul_vs_avg_txt,
                "depth_ask_over_bid_ratio": ask_bid_ratio_txt,
                "break_vol_over_prev_eaten_asks_mul": mul_vs_prev_eaten_txt,
...
def _col_name_is_buy_time(name: str) -> bool:
    n = (name or "").strip()
    if n in ("买入时间", "买入时刻", "突破时刻"):
        return True
    return "买入" in n and "时间" in n


def _resolve_summary_column_names(
    headers: List[str],
) -> Tuple[Optional[str], Optional[str], Optional[str]]:
    code_col = pick_col = buy_time_col = None
    for h in headers:
        hs = str(h or "").strip()
        if not hs:
            continue
        if code_col is None and _col_name_is_stock_code(hs):
            code_col = hs
        elif pick_col is None and _col_name_is_pick_date(hs):
            pick_col = hs
        elif buy_time_col is None and _col_name_is_buy_time(hs):
            buy_time_col = hs
    return code_col, pick_col, buy_time_col


def _parse_buy_time_to_seconds(s: str) -> Optional[float]:
    """将买入时间归一为当日 00:00 起秒数（支持 HH:MM:SS / HH:MM 及带日期前缀）。"""
    raw = (s or "").strip()
    if not raw or raw.lower() in ("nan", "none", "nat"):
        return None
    t = raw
    if "T" in t and len(t) >= 19:
        t = t.split("T", 1)[-1].strip()
    if " " in t and len(t) >= 12:
        t = t.rsplit(" ", 1)[-1].strip()
    for fmt in ("%H:%M:%S", "%H:%M"):
        try:
            dt = datetime.strptime(t[: len(fmt)], fmt)
...
def _find_tick_index_at_time(
    timed_rows: List[Tuple[float, Any]], target_sec: float
) -> Optional[int]:
    if not timed_rows or target_sec is None:
        return None
    times = [float(t) for t, _ in timed_rows]
    tgt = float(target_sec)
    i = bisect.bisect_left(times, tgt)
    if i >= len(times):
        return len(times) - 1
    if i > 0:
        if abs(times[i] - tgt) > abs(times[i - 1] - tgt):
            i -= 1
    return int(i)


def _breakthrough_detail_at_tick_index(
    timed_rows: List[Tuple[float, Any]],
    idx: int,
    stock_code: str,
    name: str,
    ...
    selection_day_disp: str,
    vol_mul: float,
    breakthrough_time_disp: str = "",
) -> Dict[str, str]:
    """在指定 tick 索引上计算突破详情（不依赖压力位锚点表）。"""
    code = (stock_code or "").strip()
    out: Dict[str, str] = {
        "code": code,
        "name": name or "",
        "buy_trade_day": buy_trade_day_disp or "",
        "选股日": (selection_day_disp or "").strip(),
        "breakthrough_time": breakthrough_time_disp or "",
        "tick_analysis_status": "",
    }
    if idx < 0 or idx >= len(timed_rows):
        out["tick_analysis_status"] = "买入时间未匹配到 tick"
        return out
    ...
    tsec, row = timed_rows[idx]
    if not out["breakthrough_time"]:
        out["breakthrough_time"] = _seconds_from_midnight_to_hhmmss(float(tsec))

    lp = _tick_scalar_to_float(row.get("lastPrice"))
    if lp is None or float(lp) <= 0:
        out["tick_analysis_status"] = "该 tick 无有效最新价"
        return out
    ...
    prev_a1p_txt = ""
    prev_a1v_txt = ""
    pressure_ref: Optional[float] = None
    if idx > 0:
        prow = timed_rows[idx - 1][1]
        apx, avo = _row_best_ask_price_vol(prow, 1.0)
        if apx is not None:
            pressure_ref = float(apx)
            prev_a1p_txt = _format_pressure_p_by_stock_code(code, apx)
        if avo is not None:
            prev_a1v_txt = _format_tick_volume_display(avo)
    ...
    if idx < len(per_vols) and per_vols[idx] is not None:
        v_break_sh = float(per_vols[idx])
    v_break_cond1 = _max_cond1_breakthrough_volume_sh(per_vols, idx)
    avg_shares: Optional[float] = float(ps) / float(pc) if pc > 0 else None
    eaten_n: Optional[int] = None
    eaten_sum_sh: Optional[float] = None
    if idx > 0:
        eaten_n, eaten_sum_sh = _break_tick_eaten_prev_ask_depth_sum_shares(
    ...
    cond2 = (
        ask_bid_ratio_cond2 is not None
        and ask_bid_ratio_cond2 < float(_TRUE_BREAKTHROUGH_MAX_ASK_OVER_BID)
    )
    cond3 = (
        v_break_sh is not None
        and eaten_sum_sh is not None
    ...
    mul_vs_avg_txt = ""
    if v_break_cond1 is not None and avg_shares is not None and float(avg_shares) > 1e-9:
        mul_vs_avg_txt = _fmt_ratio2(float(v_break_cond1) / float(avg_shares))
    ...
    mul_vs_prev_eaten_txt = ""
    if v_break_sh is not None and eaten_sum_sh is not None and float(eaten_sum_sh) > 1e-9:
        mul_vs_prev_eaten_txt = _fmt_ratio2(float(v_break_sh) / float(eaten_sum_sh))

    eaten_n_txt = str(int(eaten_n)) if eaten_n is not None and eaten_n > 0 else ""
    eaten_vol_sum_txt = ""
    if eaten_sum_sh is not None and eaten_n is not None and eaten_n > 0:
        eaten_vol_sum_txt = _breakthrough_vol_display_in_lots(eaten_sum_sh, vol_mul)

    pressure_disp = prev_a1p_txt
    if not pressure_disp and pressure_ref is not None:
        pressure_disp = _format_pressure_p_by_stock_code(code, pressure_ref)

    out.update(
        {
            "pressure_effective_used": pressure_disp,
            "last_price": _format_pressure_p_by_stock_code(code, float(lp)),
            "volume_avg_before_break": avg_before,
            "volume_break_tick": break_vol,
            "depth_bid_vol": bid_depth_txt,
            ...
            "true_breakthrough": "True" if true_break else "False",
            "true_breakthrough_remark": remark_txt,
            "tick_analysis_status": "ok",
        }
    )
    return out


BREAKTHROUGH_DETAIL_EXPORT_KEYS: List[Tuple[str, str]] = [
    ("buy_trade_day", "买入日"),
    ("breakthrough_time", "突破时刻_核验"),
    ("pressure_effective_used", "实际使用的压力位"),
    ("last_price", "突破tick最新价"),
    ...
    ("prev_tick_eaten_ask_n", "前tick被吃卖档数"),
    ("prev_tick_eaten_asks_vol_sum", "前tick被吃卖档量合计"),
    ("break_tick_trade_price", "本tick成交价"),
    ("break_tick_trade_vol", "本tick成交量"),
    ("break_vol_over_avg_mul", "突破量/均量(倍)"),
    ("depth_ask_over_bid_ratio", "委卖/委买比(取小)"),
    ("break_vol_over_prev_eaten_asks_mul", "突破量/前tick被吃卖档合计(倍)"),
    ("true_breakthrough", "真突破"),
    ("true_breakthrough_remark", "真突破备注"),
    ("tick_analysis_status", "tick分析状态"),
]


def _read_profit_summary_xlsx(filepath: str) -> Tuple[List[str], List[Dict[str, Any]]]:
    try:
        import pandas as pd
    except ImportError as e:
        raise RuntimeError(
            "读取 Excel 需要 pandas：pip install pandas openpyxl"
        ) from e
    try:
        df = pd.read_excel(filepath, dtype=object, engine="openpyxl")
    except Exception:
        try:
            ...
            raise RuntimeError(f"无法读取 Excel：{filepath}") from e
    headers = [str(c) for c in df.columns]
    rows: List[Dict[str, Any]] = []
    for _, ser in df.iterrows():
        rows.append(
            {str(c): _excel_cell_to_trimmed_str(ser.get(c)) for c in df.columns}
        )
    return headers, rows


def _export_merged_rows_to_xlsx(
    filepath: str, headers: List[str], rows: List[Dict[str, Any]]
) -> bool:
    try:
    ...
    for row in rows:
        ...
        for h in headers:
            v = row.get(h, "")
            if h in ("代码", "股票代码") and isinstance(v, str):
                line.append(_excel_stock_cell_str_for_export(v))
            else:
                line.append("" if v is None else v)
        ws.append(line)
    for c_idx, h in enumerate(headers, start=1):
        col = get_column_letter(c_idx)
        align = "center" if h in ("代码", "选股日", "买入日", "真突破") else "left"
...
def run_breakbuy_check(
    input_path: str,
    output_path: Optional[str] = None,
    progress_cb=None,
) -> Tuple[str, List[str], List[Dict[str, Any]]]:
    """
    读取汇总 xlsx，逐行计算突破详情并合并列。
    返回 (输出路径, 表头, 合并后的行)。
    """
    in_path = os.path.abspath(input_path)
    if not os.path.isfile(in_path):
        raise FileNotFoundError(in_path)
    orig_headers, rows = _read_profit_summary_xlsx(in_path)
    code_col, pick_col, buy_time_col = _resolve_summary_column_names(orig_headers)
    if not code_col or not pick_col or not buy_time_col:
        raise ValueError(
            "输入文件须含列：选股日、代码、买入时间（当前识别："
            f"代码={code_col or '-'} 选股日={pick_col or '-'} 买入时间={buy_time_col or '-'})"
        )

    detail_headers = [cn for _, cn in BREAKTHROUGH_DETAIL_EXPORT_KEYS]
    out_headers = list(orig_headers)
    for cn in detail_headers:
        if cn not in out_headers:
            out_headers.append(cn)

    merged_rows: List[Dict[str, Any]] = []
    total = len(rows)
    tick_cache: Dict[Tuple[str, str], Any] = {}

    for i, src in enumerate(rows):
        merged = dict(src)
        code_raw = src.get(code_col, "")
        code = _zfill_6(str(code_raw))
        pick_raw = src.get(pick_col, "")
        buy_time_raw = src.get(buy_time_col, "")
        pick_yyyymmdd = _parse_pick_day_to_yyyymmdd(str(pick_raw))
        buy_day = (
            _next_trading_day_after_yyyymmdd(pick_yyyymmdd)
            if pick_yyyymmdd
            else None
        )

        if not _is_valid_stock_code_6(code):
            detail = {"tick_analysis_status": "无效股票代码"}
        elif not pick_yyyymmdd:
            detail = {"tick_analysis_status": "无法解析选股日"}
        elif not buy_day:
            detail = {"tick_analysis_status": "无法计算买入日(下一交易日)"}
        else:
            target_sec = _parse_buy_time_to_seconds(str(buy_time_raw))
            if target_sec is None:
                detail = {"tick_analysis_status": "无法解析买入时间"}
            else:
                cache_key = (code, buy_day)
                tick_df = tick_cache.get(cache_key)
                if cache_key not in tick_cache:
                    tick_df = _load_or_fetch_tick_df(code, buy_day)
                    tick_cache[cache_key] = tick_df
                if tick_df is None or getattr(tick_df, "empty", True):
                    detail = {
                        "buy_trade_day": _yyyymmdd_to_display(buy_day),
                        "tick_analysis_status": "无 tick 数据",
                    }
                else:
                    timed_rows = _build_tick_timed_rows_sorted(tick_df)
                    vol_mul = _infer_tick_vol_to_shares_multiplier(tick_df)
                    idx = _find_tick_index_at_time(timed_rows, target_sec)
                    bt_disp = _seconds_from_midnight_to_hhmmss(target_sec)
                    detail = _breakthrough_detail_at_tick_index(
                        timed_rows,
                        idx if idx is not None else -1,
                        code,
                        "",
                        _yyyymmdd_to_display(buy_day),
                        _format_pick_date_cell(pick_raw),
                        vol_mul,
                        breakthrough_time_disp=bt_disp,
                    )

        for en, cn in BREAKTHROUGH_DETAIL_EXPORT_KEYS:
            merged[cn] = detail.get(en, detail.get(cn, ""))
        merged_rows.append(merged)
        if progress_cb:
            progress_cb(i + 1, total, code, merged.get("tick分析状态", ""))

    if not output_path:
        base, _ext = os.path.splitext(in_path)
        output_path = f"{base}_突破核验_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
    out_path = os.path.abspath(output_path)
    os.makedirs(os.path.dirname(out_path) or ".", exist_ok=True)
    if not _export_merged_rows_to_xlsx(out_path, out_headers, merged_rows):
        raise RuntimeError(f"写入 Excel 失败：{out_path}")
    return out_path, out_headers, merged_rows
...
def _apply_cross_sectional_score_adjustment(rows: List[dict]) -> None:
    ...
    for i, row in enumerate(rows):
        ...
        row["order_rating_legacy"] = _label_order_rating(adj_total)
        row["rating_score_legacy"] = _rating_score_from_label(row["order_rating_legacy"])
        _rate_seal_v2 = None
        try:
            from seal_rating_v2 import rate_seal_v2 as _rate_seal_v2
        except ImportError:
            try:
                from tools.seal_rating_v2 import rate_seal_v2 as _rate_seal_v2
            except ImportError:
                _rate_seal_v2 = None
        if _rate_seal_v2 is not None:
            _sr = row.get("_seal_ratio")
            _rr = row.get("_rush_ratio")
            v2 = _rate_seal_v2(
                stability=row.get("order_stability"),
                close_amt_yi=row.get("_close_amt_yi"),
                hardness_pct=(float(_sr) * 100.0) if _sr is not None else row.get("seal_hardness"),
                rush_pct=(float(_rr) * 100.0) if _rr is not None else row.get("rush_intensity"),
                trend=row.get("order_trend"),
            )
            row["order_rating"] = v2["rating_v2"]
            row["rating_score"] = int(v2["rating_v2_score"])
            row["confidence_tag"] = {3: "高", 2: "中", 1: "低"}.get(
                int(v2["rating_v2_score"]), "低"
            )
        else:
            legacy = row["order_rating_legacy"]
            if "强势" in legacy or "超强" in legacy:
                row["order_rating"], row["rating_score"] = "强", 3
            elif "中等" in legacy:
                row["order_rating"], row["rating_score"] = "中", 2
            else:
                row["order_rating"], row["rating_score"] = "弱", 1
            row["confidence_tag"] = _confidence_tag(adj_total, st_score, tr_score)
...
class BreakBuyCheckWorker(QThread):
    finished_ok = pyqtSignal(str, list, list)
    ...
    def run(self):
        try:
            out_path, headers, rows = run_breakbuy_check(
                self.input_path,
                self.output_path or None,
                progress_cb=lambda cur, tot, code, st: self.progress.emit(
                    cur, tot, code, st
                ),
            )
            self.finished_ok.emit(out_path, headers, rows)
        except Exception as e:
            self.failed.emit(str(e))


class BreakBuyCheckDialog(QDialog):
    def __init__(self, parent=None, auto_run: bool = False, input_path: str = ""):
        super().__init__(parent)
        self._auto_run = bool(auto_run)
        self._input_path = (input_path or "").strip()
        self._output_path = ""
        self._merged_headers: List[str] = []
        self._merged_rows: List[Dict[str, Any]] = []
        self._worker: Optional[BreakBuyCheckWorker] = None

        self.setWindowTitle("蚂蚁量化-突破买入核验")
        ...
        file_row.addWidget(QLabel("收益汇总 xlsx："))
        self.file_edit = QLabel("未选择文件")
        self.file_edit.setWordWrap(True)
        ...
        self.analyze_btn = QPushButton("开始核验")
        self.export_btn = QPushButton("导出结果 xlsx…")
        ...
        self.status_label = QLabel("请选择含「选股日、代码、买入时间」的汇总 xlsx")
        ...
        if self._input_path and os.path.isfile(self._input_path):
            self.file_edit.setText(self._input_path)
    ...
    def _choose_file(self):
        ...
        if path:
            ...
            self.status_label.setText("已选择文件，点击「开始核验」")

    def start_analysis(self):
        path = (self._input_path or self.file_edit.text() or "").strip()
        ...
        self._worker = BreakBuyCheckWorker(path)
        self._worker.progress.connect(self._on_progress)
        self._worker.finished_ok.connect(self._on_finished)
        self._worker.failed.connect(self._on_failed)
        self._worker.start()

    def _on_progress(self, cur: int, total: int, code: str, status: str):
        self.status_label.setText(
            f"进度 {cur}/{total}  代码 {code}  {status or ''}"
        )

    def _on_finished(self, out_path: str, headers: list, rows: list):
        self._output_path = out_path
        self._merged_headers = list(headers or [])
        self._merged_rows = list(rows or [])
        self._fill_table()
        self.analyze_btn.setEnabled(True)
        self.export_btn.setEnabled(True)
        self.status_label.setText(
            f"完成，共 {len(self._merged_rows)} 行；已写入：{out_path}"
        )
        QMessageBox.information(
            self,
            "完成",
            f"分析完成，共 {len(self._merged_rows)} 行。\n已自动保存：\n{out_path}",
        )
    ...
    def _fill_table(self):
        ...
        for r_idx, row in enumerate(rows):
            for c_idx, h in enumerate(headers):
                txt = "" if row.get(h) is None else str(row.get(h))
                self.table.setItem(r_idx, c_idx, QTableWidgetItem(txt))
        self.table.resizeColumnsToContents()

    def _export_result(self):
        if not self._merged_rows:
            QMessageBox.warning(self, "提示", "尚无结果可导出，请先运行核验。")
            return
        default_name = (
            os.path.splitext(os.path.basename(self._input_path or "export"))[0]
            + "_突破核验.xlsx"
        )
        path, _ = QFileDialog.getSaveFileName(
            ...
            "导出合并结果",
            os.path.join(
                os.path.dirname(self._input_path or os.path.abspath(__file__)),
                default_name,
            ),
            "Excel (*.xlsx)",
        )
        ...
        if not path.lower().endswith(".xlsx"):
            path += ".xlsx"
        ok = _export_merged_rows_to_xlsx(path, self._merged_headers, self._merged_rows)
        if ok:
            self._output_path = path
            QMessageBox.information(self, "导出", f"已写入：{path}")
        else:
            QMessageBox.warning(
                self, "导出失败", "写入失败，请确认已安装 openpyxl：pip install openpyxl"
            )
...
class LimitUpGeneAnalysisDialog(QDialog):
    def __init__(self, parent=None, auto_run: bool = False):
        ...
        self.export_breakthrough_btn = QPushButton("导出 Excel…")
        self.export_breakthrough_btn.setEnabled(False)
        self.export_breakthrough_btn.clicked.connect(self._export_breakthrough_excel)
        breakthrough_head_row.addWidget(self.export_breakthrough_btn)
    ...
    def _export_breakthrough_excel(self) -> None:
        """导出「突破详情」表为 .xlsx。"""
        date = getattr(self, "_limitup_date", "") or "export"
        tbl = self.breakthrough_table
        n = tbl.columnCount()
        aligns: List[str] = []
        center_headers = {
            "股票代码",
        ...
        for c in range(n):
            ...
            ht = (hi.text() if hi else "").strip()
            if ht in center_headers:
                aligns.append("center")
            elif ht in ("股票名称", "备注"):
                aligns.append("left")
            else:
    ...
    def _clear_stat_tables(self) -> None:
        ...
        if hasattr(self, "breakthrough_table"):
            self.breakthrough_table.setRowCount(0)
            self.breakthrough_table.setColumnCount(0)
        if hasattr(self, "export_breakthrough_btn"):
            self.export_breakthrough_btn.setEnabled(False)
    ...
    def _on_finished(
        ...
        if finish_kind == "list_only":
            ...
            for r_idx, brow in enumerate(bt_rows):
                for c_idx, (key, _) in enumerate(bt_headers):
                    ...
                    if key in (
                        ...
                        "prev_tick_ask1_vol",
                        "prev_tick_eaten_ask_n",
                        "prev_tick_eaten_asks_vol_sum",
                        "break_tick_trade_price",
                        "break_tick_trade_vol",
                        "break_vol_over_avg_mul",
                        "depth_ask_over_bid_ratio",
                        "break_vol_over_prev_eaten_asks_mul",
                    ):
                        item.setTextAlignment(Qt.AlignRight | Qt.AlignVCenter)
            ...
            self.export_breakthrough_btn.setEnabled(len(bt_rows) > 0)
            self._refresh_ib_export_buttons()
        else:
...
def main():
    parser = argparse.ArgumentParser(description="蚂蚁量化-突破买入核验")
    parser.add_argument(
        "--input",
        "-i",
        dest="input_path",
        default="",
        help="各日选股收益汇总 xlsx（须含列：选股日、代码、买入时间）",
    )
    parser.add_argument(
        "--output",
        "-o",
        dest="output_path",
        default="",
        help="输出 xlsx 路径（默认：输入文件名_突破核验_时间戳.xlsx）",
    )
    parser.add_argument(
        "--batch",
        action="store_true",
        help="命令行批量模式（须配合 -i）；不加此参数时默认打开 GUI",
    )
    parser.add_argument(
        "--gui",
        action="store_true",
        help="打开图形界面（默认行为，可省略）",
    )
    args, _unknown = parser.parse_known_args()

    input_path = (args.input_path or "").strip()
    if args.batch and input_path:
        try:
            out_path, _h, rows = run_breakbuy_check(
                input_path, (args.output_path or "").strip() or None
            )
            print(f"完成：{len(rows)} 行 -> {out_path}")
        except Exception as e:
    ...
    dlg = BreakBuyCheckDialog(
        auto_run=bool(args.input_path),
        input_path=input_path,
    )
    dlg.show()
    if args.input_path:
        QTimer.singleShot(300, dlg.start_analysis)
    sys.exit(app.exec_())

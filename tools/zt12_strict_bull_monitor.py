# -*- coding: utf-8 -*-
"""
涨停后1–2天严多头 · 策略表现跟踪（核心库）

用法:
  python tools/zt12_strict_bull_monitor.py
  python tools/zt12_strict_bull_monitor.py --window 10 --months 2
  python tools/zt12_strict_bull_monitor.py --variant 真突破
...
口径:
  ...
  - 未买入、未清仓样本不进窗内得分
  - 票均与「开买收益明细」同源；导出「当天买入」优先成交CSV并集，缺则由明细回填（禁止两边打架）
"""
...
DIR = ROOT / "history_data" / "涨停后1到2天严多头"
RULE_NAME = "涨停后的第1到2天-严多头"
DEFAULT_WINDOW = 10

# 回测结果分目录：选股文件仍在 DIR 根下；按票/成交明细在对应子目录
VARIANT_TRUE_BREAKTHROUGH = "真突破"
VARIANT_NO_TRUE_BREAKTHROUGH = "不看真突破"
VARIANTS = (VARIANT_TRUE_BREAKTHROUGH, VARIANT_NO_TRUE_BREAKTHROUGH)
...
def load_selection_counts() -> pd.DataFrame:
    """选股日 → 入选只数。"""
            ...
            t = pd.DataFrame(
                {
                    ...
                    "code": df[code_col].map(code6),
                    "name": df["股票名称"] if "股票名称" in df.columns else "",
                }
            )
            try:
...
def build_report(
    *,
    ...
    sel_counts = load_selection_counts()
...
def load_fills_for_trade_day(
    ...
    合并该变体下全部「回测成交明细_*.csv」中该日记录（按 mtime 新→旧），
    再按 代码+时间+方向+价格 去重。避免「最新一份 CSV 不含历史日」导致
    汇总里有开买、当天买入 sheet 却为空。
    """
    files = discover_fill_files(variant)
    if not files:
    ...
    for path in files:
        ...
        if day.empty:
            continue
        day.insert(0, "_来源文件", path.name)
        if "方向" in day.columns:
            buy_parts.append(
                day[day["方向"].astype(str).str.contains("买", na=False)].copy()
            )
            sell_parts.append(
                day[day["方向"].astype(str).str.contains("卖", na=False)].copy()
            )
        else:
            buy_parts.append(day.copy())

    def _dedupe(parts: list[pd.DataFrame]) -> pd.DataFrame:
        if not parts:
            return pd.DataFrame()
        out = pd.concat(parts, ignore_index=True)
        keys = [c for c in ("代码", "时间", "方向", "价格", "选股日") if c in out.columns]
        if keys:
            out = out.drop_duplicates(subset=keys, keep="first")
        return out.reset_index(drop=True)

    return _dedupe(buy_parts), _dedupe(sell_parts)


def _buys_sheet_from_open_detail(detail: pd.DataFrame, asof: date) -> pd.DataFrame:
    """开买收益明细 → 当天买入表（成交 CSV 缺该日时与票均同源）。"""
    if detail is None or detail.empty:
        return pd.DataFrame()
    d = detail.copy()
    code_col = "代码" if "代码" in d.columns else ("code" if "code" in d.columns else None)
    if not code_col:
        return pd.DataFrame()
    rows = []
    for _, r in d.iterrows():
        code = code6(r.get(code_col))
        if not code:
            continue
        name = r.get("名称") if "名称" in d.columns else r.get("name", "")
        sel = r.get("选股日") if "选股日" in d.columns else r.get("sel", "")
        px = r.get("买入成交价") if "买入成交价" in d.columns else r.get("px")
        rows.append(
            {
                "日期": str(asof),
                "时间": "",
                "代码": code,
                "股票名称": name,
                "选股日": str(sel)[:10] if sel is not None else "",
                "方向": "买入",
                "价格": px,
                "数量": "",
                "金额": "",
                "规则名": "",
                "触发信息": "由开买收益明细回填（成交明细 CSV 无此日买入；与票均同源）",
            }
        )
    return pd.DataFrame(rows)


def _align_buys_to_open_detail(
    buys: pd.DataFrame, detail: pd.DataFrame, asof: date
) -> pd.DataFrame:
    """当天买入与开买收益明细对齐：有 CSV 用 CSV；否则用明细回填。"""
    if detail is None or detail.empty:
        return buys if buys is not None else pd.DataFrame()
    detail_codes = set()
    code_col = "代码" if "代码" in detail.columns else ("code" if "code" in detail.columns else None)
    if code_col:
        detail_codes = {code6(x) for x in detail[code_col].tolist() if code6(x)}
    if buys is not None and not buys.empty and "代码" in buys.columns:
        buy_codes = {code6(x) for x in buys["代码"].tolist() if code6(x)}
        # CSV 已覆盖明细里全部代码 → 直接用 CSV（可含更多腿信息）
        if detail_codes and detail_codes <= buy_codes:
            return buys.reset_index(drop=True)
        # 部分缺失：保留 CSV，再补明细里缺的
        missing = detail_codes - buy_codes
        if missing:
            syn = _buys_sheet_from_open_detail(detail, asof)
            if not syn.empty and "代码" in syn.columns:
                syn = syn[syn["代码"].map(code6).isin(missing)]
                return pd.concat([buys, syn], ignore_index=True)
        return buys.reset_index(drop=True)
    return _buys_sheet_from_open_detail(detail, asof)
...
def build_asof_export_frames(
    ...
    *,
    ...
    sug_rows: list[dict] = [
        ...
        {
            "项目": "口径",
            "值": "票均=开买收益明细等权；当天买入与之同源（优先成交CSV并集，缺则由明细回填）",
        },
    ]
    ...
    frames: Dict[str, pd.DataFrame] = {
        "前一日选股列表": load_selection_rows_for_day(prev) if prev else pd.DataFrame(),
        "当日摘要与近窗": pd.DataFrame(sug_rows),
        "当日对比汇总": pd.DataFrame(compare_rows),
    }

    for v in VARIANTS:
        # 开买收益明细 = 票均样本源；当天买入必须与之对齐
        ...
        if trade_rows:
            ...
            detail = detail.rename(
                columns={k: vv for k, vv in rename.items() if k in detail.columns}
            )
        else:
            try:
                pool = load_pool(discover_trade_files(v), variant=v)
                # 与图上票均一致：按「最终已知」样本，不用开买日当天做 PIT（否则 T+1 清仓会被滤掉）
                detail = detail_for_start(pool, asof, asof=None)
                rename = {
                    "code": "代码",
                    "name": "名称",
                    "sel": "选股日",
        ...
        buys = _align_buys_to_open_detail(buys, detail, asof)
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(description="涨停后1–2天严多头表现摘要")

# -*- coding: utf-8 -*-
"""马总 ma20 单点：上MA20 全流程分析（对齐 ma5/ma10）。"""
...
OUT_ABOVE = DIR / f"各日选股收益汇总_日线-{TAG}-单点_按票_{STAMP}_收盘上MA20.xlsx"
OUT_JSON = DIR / f"_{TAG}_flow_analysis.json"
...
def prepare():
    ...
    for i, r in enumerate(df.itertuples(index=False)):
        ...
        if dd is not None and not dd.empty:
            ...
            if not sub.empty:
                cl = float(sub.iloc[-1]["close"])
        closes.append(cl)
        ma20 = r.MA20 if pd.notna(r.MA20) else None
        above.append(bool(cl is not None and ma20 is not None and cl > float(ma20)))
        if (i + 1) % 300 == 0:
    ...
    above_df = df[df["above_ma20"]].copy()
...
def analyze_factors(above: pd.DataFrame) -> dict:
    ...
    out["close_gt_ma10"] = stats(above[above["sel_close"] > above["MA10"]])
    out["close_le_ma10"] = stats(above[above["sel_close"] <= above["MA10"]])
    ...
    above = above.copy()
    above["dist_ma20"] = (above["sel_close"] - above["MA20"]) / above["MA20"] * 100
    out["corr_dist_ma20"] = round(float(above["dist_ma20"].corr(above["ret"])), 4)
    out["dist_ma20_q5"] = qcut_table(above, "dist_ma20", 5)
...
def combo_screen(above: pd.DataFrame) -> list:
    ...
    rows = [ev("基线上MA20", pd.Series(True, index=above.index))]
    for k, m in F.items():
        rows.append(ev(k, m))
    stacks = [
        ("B", ["B"]),
...
def main():
    ...
    out = {
        ...
        "above": stats(above),
        "below": stats(df[~df["above_ma20"]].assign(

# 增强选股逻辑-盘后（参照马总选股逻辑-盘后）
# 入选：全市场当日涨停（含 ST / 各板块）
# 输出：东财热门-besttest 全字段 + 满足条件 + 前十个交易日最高涨幅
# 硬过滤（不进结果表）：
#   - 非当日涨停
#   - 近 EX_DIV_LOOKBACK 个交易日（不含当日）出现疑似除权开盘缺口
#     （开盘/昨收-1 < -(涨跌停幅度+EX_DIV_EPS)；不复权下会扭曲 MA）
# 满足条件（全部为真才 True；前10日窗口不含当日）：
#   1) 当日涨停
#   2) 所属行业最高名次 ∈ (INDUSTRY_RANK_LO, INDUSTRY_RANK_HI]，
#      或所属概念最高名次 ∈ [CONCEPT_RANK_LO, CONCEPT_RANK_HI]（任一即可）
#      默认：行业 (20,40] 即名次 21–40；概念 [1,20]
#   3) 前10个交易日：主板无涨幅>=5%；创业/科创/北交所无涨幅>=10%
#   4) 当日收盘价 > MA5 且 > MA20
#   （不再要求主力净流入>=3000万；净流入字段仍输出供参考）
# 依赖引擎 ctx["em_board_hot"]（填 besttest 热门字段；不作为入选门槛）
# 近5/10/20日RS：用选股日线绝对涨幅 (close_D-close_{D-N})/close_{D-N}，对每只进表票都算；
#   与是否命中今日热门标签无关；标签内 RS 排名仍仅热门池有值（仍用近10日）
# 依赖 history_data/个股主力净流入、data/eastmoney_board_rank、data/all_a_stock_info.json
# 引擎：关闭热门池收窄，保证全市场涨停都能进表
...
HOT_MODE = "ma_zong_enhance_post_limit_up"
N = 20
# 行业名次：(LO, HI] → 默认 21–40；概念名次：[LO, HI] → 默认 1–20
INDUSTRY_RANK_LO = 20
INDUSTRY_RANK_HI = 40
CONCEPT_RANK_LO = 1
CONCEPT_RANK_HI = 20
BOARD_TOP_N_INDUSTRY = INDUSTRY_RANK_HI  # 兼容旧导出字段（展示用）
BOARD_TOP_N_CONCEPT = CONCEPT_RANK_HI
BOARD_TOP_N = BOARD_TOP_N_INDUSTRY
BOARD_SHOW_TOP_N = 30  # 表格展示名次上限（可大于入选门槛）
MIN_INFLOW_WAN = 3000.0  # 仅展示参考，不参与满足条件
REQUIRE_MIN_INFLOW = False
PRIOR_LOOKBACK = 10
...
def _board_top_hit(stock_code, sectors, as_of_date):
    ...
    # 展示用：仍收集前 show_n；满足条件的名次带在 select() 里按最高名次判定
    cond_n_ind = int(INDUSTRY_RANK_HI)
    cond_n_con = int(CONCEPT_RANK_HI)
    ...
    def _consider(tag, rk, kind):
        ...
        # 兼容旧 top-N 布尔（真正满足条件用最高名次区间，见 _cond_board_band）
...
def _cond_board_band(owned_board):
    """行业最高名次 ∈ (LO, HI] 或 概念最高名次 ∈ [LO, HI]。"""
    ind = _parse_rank_int((owned_board or {}).get("所属行业最高排名名次"))
    con = _parse_rank_int((owned_board or {}).get("所属概念最高排名名次"))
    ind_ok = (
        ind is not None
        and int(INDUSTRY_RANK_LO) < int(ind) <= int(INDUSTRY_RANK_HI)
    )
    con_ok = (
        con is not None
        and int(CONCEPT_RANK_LO) <= int(con) <= int(CONCEPT_RANK_HI)
    )
    return bool(ind_ok or con_ok)
...
def _fail_reasons_logic1(
    *,
    ...
    if not cond_board:
        reasons.append(
            "行业或概念排名不满足，要求行业名次∈(%d,%d]或概念名次∈[%d,%d]，实际%s"
            % (
                int(INDUSTRY_RANK_LO),
                int(INDUSTRY_RANK_HI),
                int(CONCEPT_RANK_LO),
                int(CONCEPT_RANK_HI),
                _board_fail_actual_detail(owned_board, board_err),
            )
        )
    if bool(REQUIRE_MIN_INFLOW) and (not cond_inflow):
...
def select(stock_code, stock_name, sectors, daily_data, as_of_date, ctx):
    ...
    is_lu, close_price, _prev = _today_limit_up(
        stock_code, stock_name, daily_data, as_of_date
    )
    if not is_lu:
        return False, {"热门模式": HOT_MODE, "_skip": "非当日涨停"}

    # 疑似除权：不进结果表（整表回测/因子筛选都不应带上）
    no_ex_div, ex_div_gap, ex_div_date = _prior_ex_div_gap(
        stock_code, stock_name, daily_data, as_of_date, EX_DIV_LOOKBACK
    )
    if not no_ex_div:
        ds = ""
    ...
    hot, mv = _hot_fields_from_ctx(stock_code, ctx)
    ...
    board_ok, best_rk, best_name, best_kind, hit_str, board_err = _board_top_hit(
        stock_code, sectors, as_of_date
    )
    bundle = _day_bundle(as_of_date)
    inflow_wan = (bundle.get("inflow_wan") or {}).get(_code6(stock_code))
    inflow_ratio = (bundle.get("inflow_ratio") or {}).get(_code6(stock_code))
    inflow_pct_float = (bundle.get("inflow_pct_of_float") or {}).get(_code6(stock_code))
    ...
    cond_board = _cond_board_band(owned_board)
    cond_inflow = bool(inflow_ok)  # 仅参考，默认不进满足条件
    cond_prior = bool(no_big_move)
    cond_ma = bool(above_ma)
    meet_all = cond_lu and cond_board and cond_prior and cond_ma
    if bool(REQUIRE_MIN_INFLOW):
        meet_all = bool(meet_all) and bool(cond_inflow)
    ...
    extra = {
        ...
        "条件_行业名次下限开": int(INDUSTRY_RANK_LO),
        "条件_行业名次上限闭": int(INDUSTRY_RANK_HI),
        "条件_概念名次下限闭": int(CONCEPT_RANK_LO),
        "条件_概念名次上限闭": int(CONCEPT_RANK_HI),
        "条件_行业前N": int(BOARD_TOP_N_INDUSTRY),
        "条件_概念前N": int(BOARD_TOP_N_CONCEPT),
        "条件_主力净流入>=3000万": bool(cond_inflow),
        "条件_要求主力净流入": bool(REQUIRE_MIN_INFLOW),
        ...
        "INDUSTRY_RANK_LO": int(INDUSTRY_RANK_LO),
        "INDUSTRY_RANK_HI": int(INDUSTRY_RANK_HI),
        "CONCEPT_RANK_LO": int(CONCEPT_RANK_LO),
        "CONCEPT_RANK_HI": int(CONCEPT_RANK_HI),
        ...
        "REQUIRE_MIN_INFLOW": bool(REQUIRE_MIN_INFLOW),

...
# 满足条件（诊断软门槛五项，不挡进池；行情类字段一律相对涨停锚点日 L）：
#   1) 涨停日所属行业东财涨幅榜前 BOARD_TOP_N_INDUSTRY，或概念前 BOARD_TOP_N_CONCEPT
#      （读 industry_rank_ / concept_rank_，与盘中逻辑2一致；非涨停家数榜）
#   2) 涨停日流通市值 < MAX_FLOAT_MV_YI（默认 80 亿）
#      优先当日/近邻「个股主力净流入」快照；更早日期用最近快照推算流通股本×涨停日收盘粗估
#   3) 涨停日前10个交易日：主板无涨幅>=5%；创业/科创/北交所无涨幅>=10%
#   4) 涨停日收盘价 > MA5 且 > MA20
#   5) 涨停日收盘价 > 布林上轨（MA20 + 2*STD20）
...
N = 20
BOARD_TOP_N_INDUSTRY = 32  # 满足条件：所属行业东财涨幅排名门槛（同盘中逻辑2）
BOARD_TOP_N_CONCEPT = 8  # 满足条件：所属概念东财涨幅排名门槛（同盘中逻辑2）
BOARD_TOP_N = BOARD_TOP_N_INDUSTRY  # 兼容旧字段（取行业门槛）
BOARD_SHOW_TOP_N = 30  # 表格展示名次上限（可大于入选门槛）
BOARD_RANK_KIND = "chg"  # chg=东财涨跌幅榜；lu=涨停家数榜
MAX_FLOAT_MV_YI = 80.0  # 满足条件：涨停日流通市值上限（亿元）
PRIOR_LOOKBACK = 10
LU_LOOKBACK = 10  # 涨停回溯交易日数（含选股日）
...
def _peak_high_ret_vs_lu_close(stock_code, daily_data, lu_date, as_of_date):
    """L 后～选股日（含）：最高价相对 L 日收盘的最大涨幅（小数）。

    与 _touched_ma_after_lu 一致：仅扫 lu_date 的下一交易日～as_of（含）。
    L=as_of 时无可扫日 → (False, None, None, lu_close, thr)。
    返回 (exceeded, peak_ret, peak_date, lu_close, thr)。
    """
    dd = _sorted_daily(daily_data, as_of_date)
    lu_d = _as_date(lu_date)
    as_d = _as_date(as_of_date)
    thr = _big_move_threshold(stock_code)
    lu_close = _asof_close(daily_data, lu_date)
    if dd is None or lu_d is None or as_d is None:
        return False, None, None, lu_close, thr
    if lu_close is None or float(lu_close) <= 0:
        return False, None, None, lu_close, thr
    if "high" not in dd.columns:
        return False, None, None, lu_close, thr
    lu_close_f = float(lu_close)
    peak_ret = None
    peak_date = None
    dates = list(dd["_d"].tolist() if "_d" in dd.columns else dd["date"].map(_as_date))
    try:
        highs = [float(x) for x in dd["high"].tolist()]
    except (TypeError, ValueError):
        return False, None, None, lu_close, thr
    for i, d in enumerate(dates):
        if d is None or d <= lu_d or d > as_d:
            continue
        try:
            high_v = float(highs[i])
        except (TypeError, ValueError, IndexError):
            continue
        if high_v != high_v or high_v <= 0:
            continue
        ret = (high_v - lu_close_f) / lu_close_f
        if peak_ret is None or ret > peak_ret:
            peak_ret = ret
            peak_date = d
        if _is_big_move_ret(ret, thr):
            return True, ret, d, lu_close, thr
    return False, peak_ret, peak_date, lu_close, thr
...
def _day_bundle(as_of_date):
    ...
    try:
        from utils.eastmoney_board_rank_ctx import (
            board_lu_rank_csv_paths,
            board_rank_csv_paths,
            _load_rank_chg_maps,
        )
        # 选股沙箱 builtins 不含 globals，直接读模块常量
        _kind = str(BOARD_RANK_KIND or "chg").strip().lower()
        if _kind == "lu":
            paths = board_lu_rank_csv_paths(as_of_date)
            _missing = "无涨停家数行业/概念榜"
        else:
            paths = board_rank_csv_paths(as_of_date)
            _missing = "无东财行业/概念涨幅榜"
    ...
    try:
        ...
        owned_maps = build_code_owned_board_rank_maps(
            as_of_date,
            rank_kind=str(BOARD_RANK_KIND or "chg"),
        )
...
def _sel_day_contrast_fields(stock_code, sectors, daily_data, as_of_date):
    ...
    if mv_d is None:
        ...
        mv_d = mv_hot_d
    if mv_d is None:
        mv_d, mv_note_d = _resolve_mv_yi(stock_code, as_d, close_d, bundle_d)
    if mv_d is not None:
        out["流通市值_亿_选股日"] = round(float(mv_d), 2)
    if mv_note_d and str(mv_note_d).startswith("scaled_from_"):
        out["流通市值备注_选股日"] = mv_note_d
    return out
...
def _fail_reasons_logic1(
    *,
    ...
    if not cond_float_mv:
        if float_mv_yi is None:
            reasons.append(
                "流通市值不满足，要求<%s亿，实际无数据"
                % _fmt_num(MAX_FLOAT_MV_YI, 0)
            )
        else:
            reasons.append(
                "流通市值不满足，要求<%s亿，实际%s亿"
                % (_fmt_num(MAX_FLOAT_MV_YI, 0), _fmt_num(float_mv_yi, 2))
            )
    if not cond_prior:
    ...
    if not cond_boll:
        reasons.append(
            "收盘站上布林上轨不满足，要求收盘>布林上轨(MA20+2*STD20)，实际收盘=%s 上轨=%s"
            % (_fmt_num(close_price, 4), _fmt_num(boll_upper, 4))
        )
    return "；".join(reasons)
...
def select(stock_code, stock_name, sectors, daily_data, as_of_date, ctx):
    ...
    if lu_date is None:
        return False, {"热门模式": HOT_MODE, "_skip": "近%d日无涨停" % int(LU_LOOKBACK)}
    ...
    hi_exceeded, hi_peak_ret, hi_peak_date, lu_close_ref, hi_thr = _peak_high_ret_vs_lu_close(
        stock_code, daily_data, lu_date, as_of_date
    )
    if hi_exceeded:
        hi_ds = ""
        peak_hit = hi_peak_ret
        peak_day = hi_peak_date
        if peak_day is not None:
            try:
                hi_ds = peak_day.strftime("%Y-%m-%d")
            except Exception:
                hi_ds = str(peak_day)
        hi_pct = "" if peak_hit is None else round(float(peak_hit) * 100.0, 4)
        thr_pct = round(float(hi_thr) * 100.0, 2)
        return False, {
            "热门模式": HOT_MODE,
            "_skip": "L后最高价超阈值",
            "涨停锚点日": lu_date.strftime("%Y-%m-%d") if hasattr(lu_date, "strftime") else str(lu_date),
            "涨停日期": lu_date.strftime("%Y-%m-%d") if hasattr(lu_date, "strftime") else str(lu_date),
            "L日收盘价": "" if lu_close_ref is None else round(float(lu_close_ref), 4),
            "L后超阈值日": hi_ds,
            "L后超阈值涨幅%": hi_pct,
            "L后最高价阈值%": thr_pct,
        }
    ...
    mv_note = ""
    if mv is None:
        mv, mv_note = _resolve_mv_yi(stock_code, lu_date, close_price, bundle)
    if mv is not None:
        hot["流通市值_亿"] = round(float(mv), 2)
    float_mv_ok = mv is not None and float(mv) < float(MAX_FLOAT_MV_YI)
    try:
    ...
    boll_upper = _boll_upper(closes, period=20, k=2.0)
    above_boll = (
        close_price is not None
        and boll_upper is not None
        and float(close_price) > float(boll_upper)
    )

    cond_board = bool(board_ok)
    cond_float_mv = bool(float_mv_ok)
    cond_prior = bool(no_big_move)
    cond_ma = bool(above_ma)
    cond_boll = bool(above_boll)
    # 软门槛不含「必须当日涨停」——硬条件已是锚点+未触
    meet_all = (
        cond_board and cond_float_mv and cond_prior and cond_ma and cond_boll
    )
    fail_reasons = _fail_reasons_logic1(
        cond_board=cond_board,
        cond_float_mv=cond_float_mv,
    ...
    extra = {
        ...
        "L后最高涨幅%": (
            ...
            else round(float(hi_peak_ret) * 100.0, 4)
        ),
        "L后最高涨幅日": (
            ""
            if hi_peak_date is None
            else (
                hi_peak_date.strftime("%Y-%m-%d")
                if hasattr(hi_peak_date, "strftime")
                else str(hi_peak_date)
            )
        ),
        "L后最高价阈值%": round(float(hi_thr) * 100.0, 2),
        "条件_行业或概念排名达标": bool(cond_board),
        "条件_行业前N": int(BOARD_TOP_N_INDUSTRY),
        "条件_概念前N": int(BOARD_TOP_N_CONCEPT),
        "条件_流通市值<80亿": bool(cond_float_mv),
        "条件_前10日无大涨": bool(cond_prior),
        "条件_收盘站上MA5且MA20": bool(cond_ma),
        "条件_收盘站上布林上轨": bool(cond_boll),
        "布林上轨": "" if boll_upper is None else round(float(boll_upper), 4),
        ...
        "流通市值备注": mv_note if mv_note else (
            "" if mv is not None else (bundle.get("mv_err") or bundle.get("inflow_err") or "")
        ),
        "TOP_N": int(TOP_N),
        "RS_TOP_K": int(RS_TOP_K),
        "RS_LO": int(RS_LO),

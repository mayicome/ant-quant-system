# -*- coding: utf-8 -*-
"""安装策略：卖：马总逻辑1-开盘涨幅单点半仓+近涨停单点半仓

参照「卖：马总逻辑1-开盘涨幅弹性半仓+近涨停弹性半仓」，唯一差异：
best_sell → single_sell（现价 >= 触发价即卖；无回落 drop_percent）。
不含 1455 破 MA20 清仓。可重复运行本脚本更新策略代码。
"""
...
STRATEGY_NAME = '卖：马总逻辑1-开盘涨幅单点半仓+近涨停单点半仓'
PREFERRED_ID = 'strategy_a3ba2d4c'

STRATEGY_CODE = r'''# 卖：马总选股逻辑1（单点卖出 · 半仓；参照弹性半仓，仅改为单点卖）
...
NAME_OPEN50 = "马总1单点卖-开盘涨幅半仓"
NAME_LU10 = "马总1单点卖-近10日涨停价半仓"
...
def run(codes, prices, get_name, account, params):
    ...
    root = _project_root()
    state_path = (
        _os.path.join(root, "data", "ma_zong1_sell_single_half_filled_legs.json")
    ...
    for code in codes or []:
        ...
        if leg_open not in filled and prev_close > 0 and half >= 100:
            open_trig = _clamp(round(prev_close * (1.0 + thr), 2), limit_down, limit_up)
        lu_trig = 0.0
        ...
        if want_open and want_lu:
            ...
            px_min = open_trig if open_trig > 0 else 0.0
            open_vol = _fold_dust(half, hold, px_min)
            lu_vol = 0
        elif want_lu:
            px_min = lu_trig if lu_trig > 0 else 0.0
...
def main() -> None:
    ...
    sp.pop("drop_percent", None)
    sp.update(
        {
            "open_gain_main": float(sp.get("open_gain_main") or 0.05),
            "open_gain_growth": float(sp.get("open_gain_growth") or 0.10),
            "entry_window_trading_days": int(sp.get("entry_window_trading_days") or 10),
            "min_order_amount": float(sp.get("min_order_amount") or 5000),
            "buy_amount_per_stock": float(sp.get("buy_amount_per_stock") or 50000),
            "sizing_mode": str(sp.get("sizing_mode") or "fixed"),

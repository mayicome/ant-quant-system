...
RANK_MEET_LO = 5  # 满足条件：行业/概念名次下限（含）
RANK_MEET_HI = 38  # 满足条件：行业/概念名次上限（含）
MAX_FLOAT_MV_YI = 80.0  # 诊断列：涨停日流通市值上限（亿元）
PRIOR_LOOKBACK = 10
...
def _rank_in_meet_band(rk):
    """行业/概念最高名次是否落在满足条件区间 [RANK_MEET_LO, RANK_MEET_HI]。"""
    if rk is None or rk == "":
        return False
    try:
        r = int(rk)
        ...
        try:
            ...
            return False
    return int(RANK_MEET_LO) <= int(r) <= int(RANK_MEET_HI)
...
def select(stock_code, stock_name, sectors, daily_data, as_of_date, ctx):
    ...
    cond_rank_mid = _rank_in_meet_band(
        owned_board.get("所属行业最高排名名次")
    ) or _rank_in_meet_band(owned_board.get("所属概念最高排名名次"))
    # 软门槛不含「必须当日涨停」——硬条件已是锚点+未触
    meet_all = bool(cond_prior and cond_ma and cond_boll and cond_rank_mid)
    fail_reasons = _fail_reasons_logic1(
        cond_rank_mid=cond_rank_mid,
    ...
    extra = {
        ...
        "条件_行业或概念排名[%d,%d]"
        % (int(RANK_MEET_LO), int(RANK_MEET_HI)): bool(cond_rank_mid),
        "条件_行业或概念中热[9,32]": bool(cond_rank_mid),  # 兼容旧列名；语义随 RANK_MEET_*
        "满足条件中热名次下限": int(RANK_MEET_LO),
        "满足条件中热名次上限": int(RANK_MEET_HI),

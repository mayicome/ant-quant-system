# -*- coding: utf-8 -*-
"""Replay MA_GAP_LO 0.5%→0.8% on 215 新规则 fills vs baseline."""
...
OUT_XLSX = ROOT / "MA_GAP_LO_0p8_单独回放.xlsx"
OUT_JSON = ROOT / "_replay_ma_gap_lo_0p8.json"
...
NAME = "股票名称"
LO = 0.008  # 0.8%
HI = 0.02  # keep upper unchanged; baseline already in [0.5%, 2%]
...
def main() -> None:
    ...
    base = df.copy()
    # raise lower bound only: gap >= 0.8% (and still <= 2% as in baseline)
    keep = df[df[GAP].notna() & (df[GAP] >= LO) & (df[GAP] <= HI)].copy()
    drop = df[df[GAP].isna() | (df[GAP] < LO)].copy()
    ...
    weak_kept = set(
        daily.loc[
            (daily["选股日"].isin(weak_days)) & (daily["gap≥0.8%笔数"] > 0), "选股日"
        ]
    )
    ...
    if m_drop["mean"] is not None and m_keep["mean"] is not None:
        if m_drop["mean"] >= m_keep["mean"]:
            verdict = "不建议收紧：被砍均收益≥保留"
        elif m_drop["mean"] > 0:
            verdict = "理由偏弱：被砍仍为正期望"
        else:
            verdict = "可考虑：被砍为负/接近零"
    else:
        verdict = "样本不足"

    summary = pd.DataFrame(
        [
            {"项": "过滤", "值": "均线差占比 ≥ 0.8%（MA_GAP_LO 0.5%→0.8%，上沿仍2%）"},
            {"项": "基线笔数", "值": m_base["n"]},
            ...
            {"项": "有票日(过滤后)", "值": int((daily["gap≥0.8%笔数"] > 0).sum())},
            {
                "项": "空仓日新增",
                "值": int(
                    ((daily["基线笔数"] > 0) & (daily["gap≥0.8%笔数"] == 0)).sum()
                ),
            },
    ...
    if len(drop):
        ...
        print("gap of cut: min/max", drop[GAP].min(), drop[GAP].max())

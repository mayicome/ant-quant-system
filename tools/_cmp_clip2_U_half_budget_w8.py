# -*- coding: utf-8 -*-
"""Compare full-equity vs half-budget for clip(2,U) · U∈{4,6,8} · w=8.

Same setup as tools/_cmp_clip2_4_half_budget_w8.py:
...
Usage:
  ...
  python tools/_cmp_clip2_U_half_budget_w8.py
"""
...
OUT_XLSX = ROOT / "clip2_U_半仓_w8_对比.xlsx"
OUT_CSV_SUMMARY = ROOT / "clip2_U_半仓_w8_汇总.csv"
OUT_CSV_FILLS = ROOT / "clip2_U_半仓_w8_成交明细.csv"
OUT_JSON = ROOT / "clip2_U_半仓_w8_对比.json"
CAPITAL0 = 100_000.0
W = 8
L_ = 2
US = (4, 6, 8)
...
def main() -> None:
    ...
    print(
        f"===== clip(2,U) U={list(US)} · w={W} · capital={CAPITAL0:.0f} · "
    ...
    print("\n===== 6行对比表 =====")
    hdr = (
        f"{'方案':<26} {'收益%':>8} {'成交':>6} {'跳过':>6} "
        f"{'均ret%':>8} {'胜率%':>7} {'maxDD%':>8}"
    )
    print(hdr)
    for r in results:
        print(
            f"{r['方案']:<26} {r['_ret_pct']:+8.2f} {r['成交笔数']:6d} {r['跳过笔数']:6d} "
            f"{r['均ret%']:+8.2f} {r['胜率%']:7.1f} {r['最大回撤%']:8.2f}"
        )

    # per-U half vs full verdict
    print("\n===== 半仓 vs 全仓（按 U） =====")
    deltas = {}
    ...
    for U in US:
        ...
        print(
            f"  clip(2,{U}): 半仓 {half['_ret_pct']:+.2f}% vs 全仓 {full['_ret_pct']:+.2f}% "
            f"（Δ{delta:+.2f}pp）→ {mark}"
        )

    if half_better_all:
        verdict = (
            f"本期样本上，半仓在 U=4/6/8 均优于全仓"
    ...
    # pivot: per clip, full vs half side columns
    pivot_rows = []
    for U in US:
        full = next(r for r in results if r["clip_U"] == U and r["budget_frac"] == 1.0)
        half = next(r for r in results if r["clip_U"] == U and r["budget_frac"] == HALF_FRAC)
    ...
    meta = pd.DataFrame(
        [
            {"项": "方法", "值": "native 新规则215 · clip(2,U) U=4/6/8 · w=8 · 10万顺序回放"},

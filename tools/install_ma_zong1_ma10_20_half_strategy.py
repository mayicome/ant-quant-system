# -*- coding: utf-8 -*-
"""安装策略：买：马总逻辑1-涨停后跌破MA10/20各1/2

参照「买：马总逻辑1-涨停后跌破MA5/10/20各1/3」（弹性 best_buy + 昨收涨幅熔断），
去掉 MA5；仅 MA10/MA20，各买单股拟买入金额的 1/2。

- 触发：均线重合点（行情「10日」「20日」），best_buy 跌破即买（rise≈0）
- 仓位：单股拟买入金额 / 2
...
STRATEGY_NAME = "买：马总逻辑1-涨停后跌破MA10/20各1/2"
PREFERRED_ID = "strategy_mz1_ma1020_half"

STRATEGY_CODE = r'''# 买：马总选股逻辑1 — 涨停后第一次跌破 MA10/MA20 各买 1/2
...
# - 腿名固定：马总1-跌破MA10 / MA20（实盘重启靠同名合并 executed）
    ...
    for code in codes or []:
        ...
        for leg_id, field, rule_name, ma_period in LEG_SPECS:
            ...
            if leg_key in filled:
                print("[马总1] skip filled %s" % leg_key)
                continue
            if _already_touched(c6, sel_d, start_d, end_d, ma_period):
                print(
                    "[马总1] skip already_touched %s (日线已触达,不落盘)"
                    % leg_key
                )
                continue
            raw = p.get(field)
...
def main() -> None:
    ...
    fixed = OUT_DIR / "strategy_ma_zong1_ma10_20_half.json"
    existing_id = None
    prev: dict = {}
    ...
    # 若全新安装，尽量从 MA5/10/20 弹性版拷贝股票池与选股日，便于对照
    if not prev and not (prev_params.get("selection_date_by_code") or prev.get("stock_codes")):
        for p in OUT_DIR.glob("strategy_*.json"):
            try:
                raw = json.loads(p.read_text(encoding="utf-8"))
            except Exception:
                continue
            if str(raw.get("name") or "") == "买：马总逻辑1-涨停后跌破MA5/10/20各1/3":
                prev = {
                    "enabled": True,
                    "stock_codes": list(raw.get("stock_codes") or []),
                    "scheduled_generate_at": raw.get("scheduled_generate_at"),
                }
                src_sp = raw.get("strategy_params") if isinstance(raw.get("strategy_params"), dict) else {}
                prev_params = dict(src_sp)
                break
    sp = {

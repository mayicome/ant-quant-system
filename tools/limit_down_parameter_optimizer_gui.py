...
class ParameterizedAnalyzer:
    ...
    def analyze_with_params(self, tick_data: pd.DataFrame, daily_data: pd.DataFrame,
        ...
        # 先使用原始分析器获取结果
        result = self.base_analyzer.analyze_limit_down_behavior_comprehensive(
            tick_data, daily_data, stock_code, None
        )
        
        # 如果结果不是跌停，直接返回
        if not result.get('is_limit_down', False):
            return result
        ...
        # 对每个行为的得分应用权重
        if 'wash_panic_weight' in params:
            adjusted_behaviors['wash_panic'] = behaviors.get('wash_panic', 0) * params['wash_panic_weight']
        else:
            adjusted_behaviors['wash_panic'] = behaviors.get('wash_panic', 0)
        ...
        if 'passive_weight' in params:
            adjusted_behaviors['passive'] = behaviors.get('passive', 0) * params['passive_weight']
        else:
            adjusted_behaviors['passive'] = behaviors.get('passive', 0)
...
class OptimizationThread(QThread):
    ...
    def __init__(self, stock_date_pairs: List[Tuple], actual_behaviors: List[str],
        ...
        self.analysis_cache = {}  # {(stock_code, date): {behaviors: {...}, is_limit_down: bool}}
        ...
        for i, (stock_code, date) in enumerate(self.stock_date_pairs):
            ...
            try:
                ...
                self.analysis_cache[cache_key] = {
                    'behaviors': result.get('behaviors', {}),
                    'is_limit_down': result.get('is_limit_down', False),
                    'behavior_names': result.get('behavior_names', {})
                }
                precompute_count += 1
    ...
    def evaluate_params(self, params: Dict, return_details: bool = False) -> float:
        ...
        # 混淆矩阵：{实际行为: {预测行为: 数量}}
        confusion_matrix = {
            'wash_panic': {'wash_panic': 0, 'distribution': 0, 'passive': 0},
            'distribution': {'wash_panic': 0, 'distribution': 0, 'passive': 0},
            'passive': {'wash_panic': 0, 'distribution': 0, 'passive': 0}
        }
        ...
        # 用于归一化的得分范围（从所有样本中收集）
        all_scores = {'wash_panic': [], 'distribution': [], 'passive': []}
        ...
        for (stock_code, date), actual_behavior in zip(self.stock_date_pairs, self.actual_behaviors):
            ...
            try:
                ...
                if not cached_result.get('is_limit_down', False):
                    continue
        ...
        for (stock_code, date), actual_behavior in zip(self.stock_date_pairs, self.actual_behaviors):
            ...
            try:
                ...
                for behavior_key in ['wash_panic', 'distribution', 'passive']:
...
class ParameterOptimizerWindow(QMainWindow):
    ...
    def init_ui(self):
        """初始化UI"""
        self.setWindowTitle("跌停板主力行为分析 - 参数优化工具")
    ...
    def init_param_table(self):
        ...
        default_params = [
            # 权重参数
            ("wash_panic_weight", 0.5, 2.0, 0.1),  # 恐慌洗盘权重
            ("distribution_weight", 0.5, 2.0, 0.1), # 出货砸盘权重
            ("passive_weight", 0.5, 2.0, 0.1),        # 被动承压权重
            # 偏移量参数
            ("wash_panic_bias", -20.0, 20.0, 5.0),  # 恐慌洗盘偏移量
            ("distribution_bias", -20.0, 20.0, 5.0), # 出货砸盘偏移量
            ("passive_bias", -20.0, 20.0, 5.0),        # 被动承压偏移量
        ]
                ...
                for line in f:
                    ...
                    if len(parts) >= 3:
                        ...
                        try:
                            ...
                            # 如果标准格式都失败，尝试处理单数字日期（如 2025-11-3）
                            if date is None:
                                # 将 2025-11-3 转换为 2025-11-03
                                date_parts = date_str.replace('/', '-').split('-')
                                if len(date_parts) == 3:
                                    year = date_parts[0]
                                    month = date_parts[1].zfill(2)
                                    day = date_parts[2].zfill(2)
                                    date_str_normalized = f"{year}-{month}-{day}"
                                    date = datetime.strptime(date_str_normalized, '%Y-%m-%d').date()
    ...
    def display_confusion_matrix(self, confusion_matrix: Dict, total: int):
        ...
        behavior_keys = ['wash_panic', 'distribution', 'passive']
    ...
    def analyze_raw_scores(self):
        ...
        try:
            ...
            # 收集所有样本的原始得分
            all_scores = {
                'wash_panic': {'恐慌洗盘': [], '出货砸盘': [], '被动承压': []},
                'distribution': {'恐慌洗盘': [], '出货砸盘': [], '被动承压': []},
                'passive': {'恐慌洗盘': [], '出货砸盘': [], '被动承压': []}
            }
            ...
            for stock_code, date in self.stock_date_pairs:
                try:
                    ...
                    # 分析
                    result = analyzer.analyze_limit_down_behavior_comprehensive(
                        tick_data, daily_data, stock_code, None
                    )
    ...
    def display_score_analysis(self, all_scores: Dict):
        ...
        for behavior_key, behavior_name in behavior_names.items():
            ...
            for actual_behavior in ['恐慌洗盘', '出货砸盘', '被动承压']:
        ...
        wash_panic_scores = all_scores['wash_panic']['恐慌洗盘']
        if wash_panic_scores:
            text += f"\n恐慌洗盘样本中，恐慌洗盘得分：\n"
            text += f"  样本数: {len(wash_panic_scores)}\n"
            text += f"  得分范围: {min(wash_panic_scores):.2f} ~ {max(wash_panic_scores):.2f}\n"
            text += f"  平均得分: {np.mean(wash_panic_scores):.2f}\n"
            text += f"  中位数得分: {np.median(wash_panic_scores):.2f}\n"
            text += f"  得分列表: {sorted(wash_panic_scores, reverse=True)}\n"
        
        # 对比其他行为在恐慌洗盘样本中的得分
        text += f"\n恐慌洗盘样本中，其他行为的得分对比：\n"
        for other_key, other_name in behavior_names.items():
            if other_key == 'wash_panic':
                continue
            other_scores = all_scores[other_key]['恐慌洗盘']
            if other_scores:
                text += f"  {other_name}: 平均 {np.mean(other_scores):.2f}, 最大 {max(other_scores):.2f}, 得分列表 {sorted(other_scores, reverse=True)}\n"
        
        # 分析为什么恐慌洗盘被误判为出货砸盘
        text += "\n" + "=" * 80 + "\n"
        text += "【误判分析】恐慌洗盘被误判为出货砸盘的原因：\n"
        text += "-" * 80 + "\n"
        
        wash_panic_distribution_scores = all_scores['distribution']['恐慌洗盘']
        if wash_panic_distribution_scores and wash_panic_scores:
            text += f"\n在恐慌洗盘样本中：\n"
            text += f"  恐慌洗盘平均得分: {np.mean(wash_panic_scores):.2f}\n"
            text += f"  出货砸盘平均得分: {np.mean(wash_panic_distribution_scores):.2f}\n"
            text += f"  得分差异: {np.mean(wash_panic_distribution_scores) - np.mean(wash_panic_scores):.2f}\n"
            if np.mean(wash_panic_distribution_scores) > np.mean(wash_panic_scores):
                text += f"  ⚠️ 问题：出货砸盘得分高于恐慌洗盘得分！\n"
            else:
                text += f"  ✓ 恐慌洗盘得分高于出货砸盘得分\n"
    ...
    def diagnose_thresholds(self):
        ...
        try:
            ...
            for idx, (stock_code, date) in enumerate(self.stock_date_pairs):
                ...
                # 只诊断跌停板样本
                if actual_behavior not in ['恐慌洗盘', '出货砸盘', '被动承压']:
                    continue
                ...
                try:
                    ...
                    # 先调用分析方法，这会添加 is_limit_down 列（方法会修改传入的DataFrame）
                    # 使用副本避免修改原始数据
                    ...
                    if not result.get('is_limit_down', False):
                        text += f"\n【样本 {limit_down_count}】{stock_code} {date}\n"
                        text += "-" * 80 + "\n"
                        text += f"⚠️ 该样本不是跌停板，跳过诊断\n"
                        text += "\n" + "=" * 80 + "\n"
                        continue
                    
                    # 获取跌停数据（使用修改后的副本）
                    limit_down_data = tick_data_copy[tick_data_copy['is_limit_down'] == True]
                    if limit_down_data.empty:
                        text += f"\n【样本 {limit_down_count}】{stock_code} {date}\n"
                        text += "-" * 80 + "\n"
                        text += f"⚠️ 无法获取跌停数据，跳过诊断\n"
                        text += "\n" + "=" * 80 + "\n"
                        continue
                    ...
                    # 计算实际值
                    ask_volumes = limit_down_data.apply(lambda row: row['askVol'][0] if isinstance(row['askVol'], list) and len(row['askVol']) > 0 else 0, axis=1)
                    avg_ask1_vol = ask_volumes.mean() if len(ask_volumes) > 0 else 0
                    
                    # 计算跌停期间的平均单笔成交量（使用差值，因为volume是累计值）
                    if len(limit_down_data) > 1:
                        volume_diff = limit_down_data['volume'].diff().fillna(0)
                        avg_volume = volume_diff.mean() if len(volume_diff) > 0 else 0
                    elif len(limit_down_data) == 1:
                        avg_volume = limit_down_data['volume'].iloc[0]
                    else:
                        avg_volume = 0
                    
                    volume_ratio = (avg_volume / avg_ask1_vol * 100) if avg_ask1_vol > 0 else 0  # 转换为百分比
                    
                    # 计算封单稳定性
                    ask_volumes_all = limit_down_data.apply(lambda row: sum(row['askVol']) if isinstance(row['askVol'], (list, tuple, np.ndarray)) else 0, axis=1)
                    if len(ask_volumes_all) > 0:
                        vol_std = ask_volumes_all.std()
                        vol_mean = ask_volumes_all.mean()
                        volatility_ratio = (vol_std / vol_mean) if vol_mean > 0 else 1.0
                    else:
                        volatility_ratio = 1.0
                    
                    # 封板时间
                    if len(limit_down_data) > 0 and len(tick_data) > 0:
                        first_limit_down_time = limit_down_data.index[0]
                        tick_start_time = tick_data.index[0]
                        if isinstance(first_limit_down_time, pd.Timestamp) and isinstance(tick_start_time, pd.Timestamp):
                            time_to_limit = (first_limit_down_time - tick_start_time).total_seconds() / 60
                        else:
                            time_to_limit = None
                    else:
                        time_to_limit = None
                    ...
                    # 获取得分（已经在上面调用过了）
                    wash_panic_score = result.get('behaviors', {}).get('wash_panic', 0)
                    
                    # 输出诊断信息
                    text += f"\n【样本 {limit_down_count}】{stock_code} {date}\n"
                    text += "-" * 80 + "\n"
                    text += f"总流通股: {total_shares_wan:.2f} 万股\n"
                    text += f"实际卖一量: {avg_ask1_vol:.0f} 手\n"
                    ...
                    text += f"\n阈值满足情况:\n"
                    # 检查各个阈值
                    if avg_ask1_vol >= huge_seal_threshold:
                        text += f"  ✓ 满足超大封单阈值 ({avg_ask1_vol:.0f} >= {huge_seal_threshold:.0f})\n"
                    elif avg_ask1_vol >= big_seal_threshold:
                        text += f"  ✓ 满足大封单阈值 ({avg_ask1_vol:.0f} >= {big_seal_threshold:.0f})\n"
                    elif avg_ask1_vol >= large_seal_threshold:
                        text += f"  ✓ 满足大封单阈值 ({avg_ask1_vol:.0f} >= {large_seal_threshold:.0f})\n"
                    elif avg_ask1_vol >= medium_seal_threshold_high:
                        text += f"  ✓ 满足中等封单阈值 ({avg_ask1_vol:.0f} >= {medium_seal_threshold_high:.0f})\n"
                    elif avg_ask1_vol >= medium_seal_threshold_low:
                        text += f"  ✓ 满足中等封单阈值 ({avg_ask1_vol:.0f} >= {medium_seal_threshold_low:.0f})\n"
                    elif avg_ask1_vol >= small_seal_threshold:
                        text += f"  ✓ 满足小封单阈值 ({avg_ask1_vol:.0f} >= {small_seal_threshold:.0f})\n"
                    else:
                        text += f"  ✗ 不满足任何封单阈值 ({avg_ask1_vol:.0f} < {small_seal_threshold:.0f})\n"
                    ...
                    text += f"\n最终得分: {wash_panic_score} 分\n"
                    text += "\n" + "=" * 80 + "\n"
                    
                    if limit_down_count % 3 == 0:
                        self.score_analysis_text.setText(text)
                        QApplication.processEvents()
                    
                except Exception as e:
                    import traceback
                    text += f"\n【样本 {limit_down_count}】{stock_code} {date} - 分析失败: {str(e)}\n"
                    text += traceback.format_exc() + "\n"
                    continue
            
            text += f"\n诊断完成，共分析了 {limit_down_count} 个跌停板样本\n"
            self.score_analysis_text.setText(text)

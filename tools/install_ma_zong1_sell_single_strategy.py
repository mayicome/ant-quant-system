...
逻辑相同：single_sell（现价 >= 触发价即卖）；差异仅在开盘涨幅 / 近涨停两腿的仓位比例。
总仓位=本轮累计买入；腿仓按总仓位；卖出不降总仓位；剩余不够一手/最小金额则并入。
...
# (策略名, 仓位比例, 仓位中文, 开盘腿名, 近涨停腿名, 开盘leg_id, 近涨停leg_id, 落盘文件名, 优先沿用id)
SPECS = (
    (
        "卖：马总逻辑1-开盘涨幅单点半仓+近涨停单点半仓+1455破MA20清仓",
        0.5,
        ...
        "OPEN50",
        "LU10",
        "ma_zong1_sell_single_filled_legs.json",
        "strategy_32cf91e7",
    ),
    (
        "卖：马总逻辑1-开盘涨幅单点全仓+近涨停单点全仓+1455破MA20清仓",
        ...
        "OPEN100",
        "LU10",
        "ma_zong1_sell_single_full_filled_legs.json",
        "",
    ),
    (
        "卖：马总逻辑1-开盘涨幅单点三分之一仓+近涨停单点三分之一仓+1455破MA20清仓",
        ...
        "OPEN33",
        "LU10",
        "ma_zong1_sell_single_third_filled_legs.json",
        "",
    ),
)
...
def _strategy_code(
    *,
    ...
    # 半仓：两腿合计约等于总仓；全仓：两腿各自按满仓挂（先成交者清空）；三分之一：各 1/3，合计约 2/3
    frac_lit = repr(float(frac))
    return f'''# 卖：马总选股逻辑1（单点卖出版 · {frac_label}）
# 总仓位=本轮累计买入（positions_baseline）；{frac_label}按总仓位；对照当前持股并入零头/最小金额
# params.positions=可卖；positions_volume=当前持股；positions_baseline=总仓位基准
# N = params.scheduled_clear_on_sell_day / sell_hold_trading_days / entry_window_trading_days

NAME_OPEN = {name_open!r}
NAME_LU = {name_lu!r}
LEG_OPEN_ID = {leg_open_id!r}
LEG_LU_ID = {leg_lu_id!r}
LEG_FRAC = {frac_lit}
...
def run(codes, prices, get_name, account, params):
    ...
    def _name_to_leg(c6, rule_name):
        ...
        if n == NAME_OPEN:
            return "%s:%s" % (c6, LEG_OPEN_ID)
        if n == NAME_LU:
            return "%s:%s" % (c6, LEG_LU_ID)
        return None
    ...
    def _frac_lot(total):
        """按 LEG_FRAC 取整手；不足一手但总仓>=100 时至少一手（与半仓版一致）。"""
        total = _full_lot(total)
        if total < 100:
            return 0
        if LEG_FRAC >= 0.999:
            return total
        lots = int(total * LEG_FRAC / 100) * 100
        return max(100, lots)

    def _half_pair_absorb_dust(total):
        """半仓专用：两笔合计等于总仓；取整零头并入第二笔。"""
        total = _full_lot(total)
        if total < 100:
            return 0, 0
        a = _frac_lot(total)
    ...
    def _pair_vols(total, hold):
        """两腿同时挂单时的数量分配。"""
        hold = _full_lot(hold)
        piece = _frac_lot(total)
        if LEG_FRAC >= 0.999:
            # 全仓：两腿各自按当前持股满仓挂，先成交者清空
            v = hold
            return v, v
        if abs(LEG_FRAC - 0.5) < 1e-9:
            open_vol, lu_vol = _half_pair_absorb_dust(total)
        else:
            open_vol, lu_vol = piece, piece
        pair_cap = hold
    ...
    for code in codes or []:
        ...
        leg_open = "%s:%s:%s" % (c6, LEG_OPEN_ID, trade_d.strftime("%Y%m%d"))
        leg_lu = "%s:%s" % (c6, LEG_LU_ID)
        piece = _frac_lot(base)

        thr = _open_thr(c6)
        open_trig = 0.0
        if leg_open not in filled and open_px > 0 and piece >= 100:
            open_trig = _clamp(round(open_px * (1.0 + thr), 2), limit_down, limit_up)
        lu_trig = 0.0
        if leg_lu not in filled and piece >= 100:
        ...
        if want_open and want_lu:
            open_vol, lu_vol = _pair_vols(base, hold)
        elif want_open:
            open_vol = _fold_dust(piece, hold, open_trig)
            lu_vol = 0
        elif want_lu:
            lu_vol = _fold_dust(piece, hold, lu_trig)
            open_vol = 0
        else:
            open_vol, lu_vol = 0, 0
        ...
        if hold_n is not None and avail >= 100:
            result.append({{
                ...
                "stock_name": name,
                "rule_type": "scheduled_clear",
                "name": "马总1单点卖-第%d日无条件清仓" % int(hold_n),
                "price": 0.0,
                "volume": int(avail),
...
def _find_by_name(name: str) -> tuple[str | None, dict]:
    existing_id = None
    prev: dict = {}
    for p in OUT_DIR.glob("strategy_*.json"):
        ...
        if existing_id and p.name != ("%s.json" % existing_id):
            try:
                ...
                pass
        break
    return existing_id, prev


def _install_one(spec: tuple) -> None:
    name, frac, frac_label, name_open, name_lu, leg_open, leg_lu, state_file, prefer_id = spec
    existing_id, prev = _find_by_name(name)
    sid = existing_id or prefer_id or ("strategy_" + uuid.uuid4().hex[:8])
    if not existing_id and prefer_id:
        prefer_path = OUT_DIR / ("%s.json" % prefer_id)
        if prefer_path.is_file():
            try:
                prev = json.loads(prefer_path.read_text(encoding="utf-8"))
            except Exception:
                prev = {}
    # 新策略默认沿用半仓版的股票池/参数
    if not prev:
        half_path = OUT_DIR / "strategy_32cf91e7.json"
        if half_path.is_file():
            try:
                half = json.loads(half_path.read_text(encoding="utf-8"))
                prev = {
                    "enabled": True,
                    "stock_codes": list(half.get("stock_codes") or []),
                    "strategy_params": dict(half.get("strategy_params") or {}),
                    "scheduled_generate_at": half.get("scheduled_generate_at"),
                }
            except Exception:
                prev = {}

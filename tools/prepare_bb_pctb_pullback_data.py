# -*- coding: utf-8 -*-
"""布林%b回落选股 · 准备数据（近 N 个交易日选股 + bb_pctb 买/卖回测）

选股规则：布林%b回落选股
买入：次日开盘（strategy_bb_pctb_buy）
卖出：斜率/%b/12日强清（strategy_bb_pctb_sell）

用法:
  python tools/prepare_bb_pctb_pullback_data.py
  python tools/prepare_bb_pctb_pullback_data.py --days 15
  python tools/prepare_bb_pctb_pullback_data.py --force-days 30 --no-reuse

输出:
  history_data/布林%b回落选股/选股结果_布林%b回落选股_{start}_{end}.xls
  history_data/布林%b回落选股/各日选股收益汇总_日线-bb_pctb-bb_pctb_sell-单点_按票_latest.xlsx
"""
...
OUT_DIR = ROOT / "history_data" / "布林%b回落选股"
RULE_NAME = "布林%b回落选股"
DEFAULT_DAYS = 15
SELL_HOLD = 12
    ...
    *,
    ...
    try:
        ...
        if files:
            ...
            df = pd.read_excel(files[0], engine="xlrd")
            if "选股日" in df.columns and not df.empty:
                s = pd.to_datetime(df["选股日"], errors="coerce").dropna()
                if not s.empty:
                    sel_file_last = s.max().date()
    except Exception:
        sel_file_last = None
...
def prepare(
    *,
    ...
    if skip_select:
        ...
        found: set = set()
        if no_reuse:
            _log("已指定 --no-reuse：不复用旧选股，窗口内全部重选（含买点新列）", progress)
            missing = list(win)
        else:
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(description="布林%b回落监控数据准备：近N日选股 + bb_pctb 回测")
    ap.add_argument(
        "--days",
        type=int,
    ...
    ap.add_argument(
        "--no-reuse",
        action="store_true",
        help="不复用旧选股文件，窗口内全部重选（改买点规则后建议加）",
    )

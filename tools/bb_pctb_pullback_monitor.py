# -*- coding: utf-8 -*-
"""
布林%b回落选股 · 买点 A/B 对比监控（核心库）

按回测按票文件中的「买点」列，将同一回测池拆成两组对比：
  - 买点A（深度超跌反弹：近5日曾 %b<0，且今日是其后首次 %b>=0.05）
  - 买点B（下轨附近企稳：今日 %b<=0.04 且 T/T-1 不再创新低）

用法:
  python tools/bb_pctb_pullback_monitor.py
  python tools/bb_pctb_pullback_monitor.py --window 10 --months 2

目录:
  history_data/布林%b回落选股/
    选股结果_布林%b回落选股_*.xls
...
PAIR_TAG = "bb_pctb-bb_pctb_sell"
SELL_HOLD = 12


def _norm_buy_point(v: object) -> str:
...
def _latest_fill_csv(side_zh: str) -> Optional[Path]:
    side = str(side_zh or "").strip()
    for pat in (
        f"回测成交明细_日线-{PAIR_TAG}*{side}_latest.csv",
        f"回测成交明细_日线-bb_pctb*{side}_latest.csv",
    ):
        hits = sorted(DIR.glob(pat), key=lambda p: p.stat().st_mtime, reverse=True)
        if hits:
            return hits[0]
    hits = sorted(DIR.glob(f"回测成交明细_*{side}_*.csv"), key=lambda p: p.stat().st_mtime, reverse=True)
    return hits[0] if hits else None


def _pick_selection_file_for_merge() -> Optional[Path]:
    cands = discover_selection_files()
    return cands[0] if cands else None


def ensure_per_stock_xlsx(*, force: bool = False) -> Optional[Path]:
    """缺少按票 xlsx 时，从买卖 latest CSV 重建（并尽量回填选股文件列）。"""
    DIR.mkdir(parents=True, exist_ok=True)
    latest = DIR / f"各日选股收益汇总_日线-{PAIR_TAG}-单点_按票_latest.xlsx"
    if latest.is_file() and not force:
        return latest

    buy_p = _latest_fill_csv("买入")
    sell_p = _latest_fill_csv("卖出")
    ...
    try:
        from tools.merge_backtest_trades_by_selection import (
            ...
            annotate_sample_completion,
            resolve_last_available_trade_date,
        )
        from tools.run_ma_zong1_single_daily_backtest import _norm_code6, _write_stock_xlsx, _publish_latest
    except Exception as ex:
        print(f"重建按票汇总失败（导入）: {ex}", flush=True)
        return latest if latest.is_file() else None

    rows = aggregate(buy_p, sell_p)
    apply_end_date_from_trading_calendar(rows, from_t1=True, hold_n=SELL_HOLD + 1)
    prices_by_mark, price_warn = _build_prices_by_mark_date(
        rows, mark_n=SELL_HOLD + 1, use_nth_trading_day=False
    )
    apply_mark_and_returns(
        rows,
        prices_by_mark,
    ...
    sel_path = _pick_selection_file_for_merge()
    if sel_path is not None:
        try:
            msg = apply_selection_file_fields(rows, sel_path)
            print(msg, flush=True)
        except Exception as ex:
            print(f"选股列回填失败: {ex}", flush=True)

    last_d = resolve_last_available_trade_date()
    annotate_sample_completion(rows, last_available=last_d)
    ...
    stamp = buy_p.stem.split("_")[-1] if "_" in buy_p.stem else "rebuild"
    stock_xlsx = DIR / f"各日选股收益汇总_日线-{PAIR_TAG}-单点_按票_{stamp}.xlsx"
    _write_stock_xlsx(stock_xlsx, sdf)
    _publish_latest(stock_xlsx, latest)
    print(f"已重建按票汇总: {latest.name}（{len(sdf)} 行）", flush=True)
    return latest


def _buy_point_series(df: pd.DataFrame) -> pd.Series:
    if df is None or df.empty or "buy_pt" not in df.columns:
        return pd.Series(dtype=str)
    return df["buy_pt"].astype(str).str.strip().str.upper()


def _legacy_buy_point_mode(df: pd.DataFrame) -> bool:
    """旧版选股无买点列时，两组暂用同一全池展示。"""
    bp = _buy_point_series(df)
    if bp.empty:
        return True
    return not ((bp == "A").any() or (bp == "B").any())


def discover_trade_files() -> list[Path]:
    """优先 bb_pctb 买/卖 latest 按票汇总。"""
    ensure_per_stock_xlsx()
    pats = (
        "各日选股收益汇总_日线-bb_pctb-bb_pctb_sell-单点_按票_latest.xlsx",
        "各日选股收益汇总_日线-bb_pctb-bb_pctb_sell-单点_按票_已完成_latest.xlsx",
        "各日选股收益汇总_日线-bb_pctb-bb_pctb_sell-单点_按票*.xlsx",
        "各日选股收益汇总_日线-bb_pctb*-单点_按票*.xlsx",
    )
    seen: set[Path] = set()
    out: list[Path] = []
    for pat in pats:
...
def discover_selection_files() -> list[Path]:
    ...
    return sorted(cands, key=lambda p: -float(p.stat().st_mtime))
...
def _order_fill_files(files: list[Path]) -> list[Path]:
    ...
    prefer = [p for p in files if "bb_pctb" in p.name]
    others = [p for p in files if p not in prefer]
    return prefer + others


def _fill_files_for_side(side_zh: str) -> list[Path]:
    """成交明细：优先 bb_pctb latest。"""
    side = str(side_zh or "").strip()
    for pat in (
        f"回测成交明细_日线-bb_pctb-bb_pctb_sell*{side}_latest.csv",
        f"回测成交明细_日线-bb_pctb*{side}_latest.csv",
    ):
        hits = sorted(DIR.glob(pat), key=lambda p: p.stat().st_mtime, reverse=True)
        if hits:
...
def _filter_variant(df: pd.DataFrame, variant: str) -> pd.DataFrame:
    if df is None or df.empty:
        return df
    if "buy_pt" not in df.columns:
        return df.copy()
    if _legacy_buy_point_mode(df):
        return df.copy()
    bp = _buy_point_series(df)
    if variant == VARIANT_A:
        return df.loc[bp == "A"].copy()
    if variant == VARIANT_B:
        return df.loc[bp == "B"].copy()
    return df.copy()
...
def load_one(path: Path) -> pd.DataFrame:
    ...
    if BUY_COL in df.columns:
        out["buy_pt"] = df[BUY_COL].map(_norm_buy_point)
    elif "buy_pt" in df.columns:
        out["buy_pt"] = df["buy_pt"].map(_norm_buy_point)
    else:
        out["buy_pt"] = ""
    if "%b" in df.columns:
        out["pctb"] = pd.to_numeric(df["%b"], errors="coerce")
...
def load_selection_counts(variant: str | None = None) -> pd.DataFrame:
    ...
    for p in files:
        ...
        if BUY_COL in df.columns:
            t["buy_pt"] = df[BUY_COL].map(_norm_buy_point)
        else:
            t["buy_pt"] = ""
        if code_col:
    ...
    if variant == VARIANT_A:
        all_d = all_d.loc[all_d["buy_pt"] == "A"].copy()
    elif variant == VARIANT_B:
        all_d = all_d.loc[all_d["buy_pt"] == "B"].copy()
    if variant and _legacy_buy_point_mode(all_d):
        # 旧版无买点：量柱用全池
        pass
    if all_d.empty:
        return pd.DataFrame(columns=["sel", "n_sel"])
...
def build_report(
    *,
    ...
    variant = str(variant or VARIANT_A).strip() or VARIANT_A
    ...
    if df.empty:
        return _empty_report(
            ...
            last_close=last_close,
            variant=variant,
            reason=f"「{variant}」无样本（按票文件缺少{BUY_COL}列或该组为空）",
        )
    ...
    legacy = _legacy_buy_point_mode(df)
    return {
        "files": [p.name for p in files],
        "df": df,
        "legacy_buy_point": legacy,
        "day": day,
        "day_view": day_view,
        "roll": roll,
        "roll_view": roll_view,
...
def load_selection_rows_for_day(sel_day: date, *, variant: str | None = None) -> pd.DataFrame:
    ...
    for p in files:
        ...
        if variant and BUY_COL in sub.columns:
            bp = sub[BUY_COL].map(_norm_buy_point)
            if variant == VARIANT_A:
                sub = sub.loc[bp == "A"].copy()
            elif variant == VARIANT_B:
                sub = sub.loc[bp == "B"].copy()
            elif variant and not ((bp == "A").any() or (bp == "B").any()):
                pass
...
def build_asof_export_frames(
    ...
    *,
    ...
    frames: Dict[str, pd.DataFrame] = {
        "前一日选股_买点A": (
            load_selection_rows_for_day(prev, variant=VARIANT_A) if prev else pd.DataFrame()
        ),
        "前一日选股_买点B": (
            load_selection_rows_for_day(prev, variant=VARIANT_B) if prev else pd.DataFrame()
        ),

# -*- coding: utf-8 -*-
"""Cond123 回踩 (高开回踩) anytag vs besttest — clip(L,U) offline replay.

Same capital / Seff / selection-order rules as tools/_sim_cond123_clip2_4.py,
but keep slots = 高开回踩 only (单点买入), not 开盘夹档.

  - S = 当日选股池只数（全量选股结果）
  - Seff = clip(S, L, U)
  - 当日最多买 Seff 只：按选股池顺序截断 Cond123 回踩合格成交
  - spend = min(equity/Seff, cash)

Usage:
  python tools/_sim_cond123_pullback_clip.py
  python tools/_sim_cond123_pullback_clip.py --clips 2,4
"""
...
MULTI_SUMMARY_NAME = "Cond123_回踩_anytag_vs_besttest_clip2_2_2_3_2_4_2_5_对比.xlsx"
...
ANY_TRADES = f"{PREFIX}_各日选股收益汇总.xlsx"
BEST_TRADES = f"besttest_{PREFIX}_各日选股收益汇总.xlsx"
ANY_SEL = "选股结果_东财热门-anytag全量-无个股过滤_2026-07-01_2026-07-31.xls"
BEST_SEL = "选股结果_东财热门-besttest全量-无个股过滤_2026-07-01_2026-07-31.xls"

# open-clip besttest clip(2,4) reference (from prior Cond123 open sim)
OPEN_CLIP24_BEST_RET = 8.749543784866876
...
def load_sel(path: Path):
    ...
    return df, order, smap


def load_trades(path: Path, order: dict) -> pd.DataFrame:
    df = pd.read_excel(path)
...
def keep_pullback_slots(g: pd.DataFrame, Seff: int) -> pd.DataFrame:
    """回踩通道：仅高开回踩，按池序截断至 Seff。"""
    gaps = g[g["分支"] == "高开回踩"].sort_values(["池序", "t", "代码"])
    return gaps.head(Seff).reset_index(drop=True)
...
def sim(trades: pd.DataFrame, smap: dict, L_: int, U_: int, capital0: float, label: str):
    ...
    return {"stats": stats, "fills": fills_df, "skips": skips_df, "curve": curve}
...
def export_clip(results: dict, L_: int, U_: int) -> Path:
    ...
    for key, r in results.items():
        prefix = f"Cond123_回踩_{key}_{tag}"
        _safe_write(
    ...
    out_xlsx = ROOT / f"Cond123_回踩_anytag_vs_besttest_{tag}_对比.xlsx"
    ...
    meta = {
        ...
        "channel": "高开回踩",
        "note": (
            "基于 Cond123 回踩成交回放；非 GUI/QMT 全引擎重跑。"
            f"Cond2=成交相对买入日MA5∈[0,2]%（非开盘相对）。"
            f"截断规则=选股池文件顺序；S=当日全量选股池只数；Seff=clip(S,{L_},{U_})；"
            "仓位=equity/Seff；仅买高开回踩。"
        ),
        "L": L_,
        "U": U_,
        "capital0": CAPITAL0,
        "date_range": "2026-07-01..2026-07-31",
        "open_clip24_besttest_ret_pct_ref": OPEN_CLIP24_BEST_RET,
        "inputs": {
        ...
        "outputs": {
            ...
            "anytag_prefix": f"Cond123_回踩_anytag_{tag}_*",
            "besttest_prefix": f"Cond123_回踩_besttest_{tag}_*",
        },
    }
    meta_path = ROOT / f"Cond123_回踩_anytag_vs_besttest_{tag}_meta.json"
    _safe_write(
        meta_path,
...
def summary_row(st: dict) -> dict:
    return {
        ...
        "vs开盘夹档best_clip24_pp": round(st["ret_pct"] - OPEN_CLIP24_BEST_RET, 4),
    }
        ...
        delta_rows.append(
            {
                ...
                "open_clip24_best_ref%": OPEN_CLIP24_BEST_RET,
            }
        )
    delta = pd.DataFrame(delta_rows)

    tag_bits = "_".join(f"{L}_{U}" for L, U in clips)
    out_xlsx = ROOT / f"Cond123_回踩_anytag_vs_besttest_clip{tag_bits}_对比.xlsx"
...
def main():
    ap = argparse.ArgumentParser(description="Cond123 回踩 clip(L,U) anytag vs besttest")
    ap.add_argument(
        "--clips",
        nargs="*",
        default=None,
    ...
    for key, sel_name, tr_name in schemes:
        _, order, smap = load_sel(ROOT / sel_name)
        trades = load_trades(ROOT / tr_name, order)
    ...
    for L_, U_ in clips:
        ...
        sa, sb = results["anytag"]["stats"], results["besttest"]["stats"]
        print(f"===== Cond123 回踩 clip({L_},{U_}) · 10万 · 离线回放 =====")
        print(
            f"规则: Seff=clip(S,{L_},{U_}); 按选股池顺序最多买 Seff 只高开回踩; "
            "spend=min(equity/Seff,cash); Cond2=成交相对MA5"
        )
        print(fmt(sa))
        print(fmt(sb))
        print(
        ...
        print(
            f"vs 开盘夹档 besttest clip(2,4)={OPEN_CLIP24_BEST_RET:+.2f}%: "
            f"any Δ={sa['ret_pct']-OPEN_CLIP24_BEST_RET:+.2f}pp "
            f"best Δ={sb['ret_pct']-OPEN_CLIP24_BEST_RET:+.2f}pp"
        )
        print(f"导出: {out_xlsx}")
        print()

...
# -*- coding: utf-8 -*-
"""
涨停板结构分析：按所选股票池做封板/tick 结构分析。

输入来源：
1) 全部今日涨停：按「分析日期」解析当日涨停池（剔除 ST）
   优先 history_data/涨停板数据_YYYY-MM-DD.csv（非空），
   其次 涨停日数据 JSON，再回退 daily_change_*.xlsx 按板块涨停阈值筛选。
2) 指定股票列表文件：txt/csv 每行或首列为代码
3) 直接输入代码：逗号分隔

Tick 优先读本地 data/ticks；缺失时再尝试大 QMT（xtdata），不以 miniQMT 为默认依赖。
"""
    ...
    if tick_df is not None:
        try:
            ...
            if norm is not None and len(norm) > 0:
                write_tick_cache(stock_code_6, trade_date_yyyymmdd, norm)
                return norm
        except Exception:
            pass
    return tick_df
...
def _to_dashed_date(yyyymmdd: str) -> str:
    ...
    if len(s) != 8 or not s.isdigit():
        raise ValueError(f"无效日期: {yyyymmdd}")
    return f"{s[:4]}-{s[4:6]}-{s[6:8]}"


def _history_search_roots(history_dir: str) -> List[str]:
    try:
        from utils.history_data_archive import archive_search_roots

        return list(archive_search_roots(history_dir))
    except Exception:
        return [history_dir]


def _csv_looks_nonempty(filepath: str) -> bool:
    """排除仅 BOM/空表头的占位文件（监控在无数据时可能写出 5 字节空文件）。"""
    try:
...
def _find_limitup_csv_for_date(history_dir: str, yyyymmdd: str) -> str:
    """精确匹配分析日涨停 CSV（现行目录优先，再存档）；空/占位文件视为不存在。"""
    try:
        ds = _to_dashed_date(yyyymmdd)
    except ValueError:
        return ""
    name = f"涨停板数据_{ds}.csv"
    for root in _history_search_roots(history_dir):
        path = os.path.join(root, name)
        if _csv_looks_nonempty(path):
            return path
    return ""
...
def _find_latest_limitup_file(history_dir: str, today_yyyymmdd: str) -> Tuple[str, str]:
    """
    找到 history_dir（含存档）下 <= today_yyyymmdd 的最新非空涨停 CSV：
    ...
    for root in _history_search_roots(history_dir):
        ...
        for fname in names:
            ...
            if not _csv_looks_nonempty(path):
                continue
    ...
    for enc in encodings:
        try:
            with open(filepath, "r", encoding=enc, newline="") as f:
                reader = csv.DictReader(f)
                fields = [x for x in (reader.fieldnames or []) if x and str(x).strip()]
                if not fields:
                    return {}
                code_col = "代码" if "代码" in fields else fields[0]
                name_col = "名称" if "名称" in fields else None
                out: Dict[str, str] = {}
                for row in reader:
...
def _load_limitup_codes_from_day_json(history_dir: str, yyyymmdd: str) -> Tuple[Dict[str, str], str]:
    """从 history_data/涨停日数据/YYYY-MM-DD.json 读取；返回 (code->name, path_or_empty)。"""
    try:
        from utils.limit_up_day_path import resolve_limit_up_day_json_path

        path = resolve_limit_up_day_json_path(_to_dashed_date(yyyymmdd), history_dir) or ""
    except Exception:
        path = ""
    ...
    if isinstance(data, dict):
        stocks = data.get("limit_up_stocks") or data.get("stocks") or []
    out: Dict[str, str] = {}
    if isinstance(stocks, list):
        for item in stocks:
            if not isinstance(item, dict):
                continue
            code = _zfill_6(item.get("code") or "")
            if not _is_valid_stock_code_6(code):
                continue
            out[code] = str(item.get("name") or "").strip()
    return out, path


def _load_limitup_codes_from_daily_change(history_dir: str, yyyymmdd: str) -> Tuple[Dict[str, str], str]:
    """
    用本地日涨幅表近似还原当日涨停池（CSV/JSON 缺失或为空时的回退）。
    判定：涨跌幅% >= 板块涨停幅度 * 99%。
    """
    try:
        ds = _to_dashed_date(yyyymmdd)
        ...
        return {}, ""
    candidates = [
        os.path.join(history_dir, f"daily_change_{ds}.xlsx"),
        os.path.join(history_dir, f"daily_change_clean_{ds}.xlsx"),
    ]
    ...
    out: Dict[str, str] = {}
    try:
        col_max = float(pd.to_numeric(df[pct_col], errors="coerce").abs().max())
    except Exception:
        col_max = 100.0
    # 列最大值 ≤1.5 视为涨跌比(0.1=10%)；否则视为百分数(10=10%)
    pct_as_ratio = bool(col_max <= 1.5)

    for _, row in df.iterrows():
        code = _zfill_6(row.get(code_col))
        if not _is_valid_stock_code_6(code):
            continue
        try:
        ...
        if pct_as_ratio:
            pct *= 100.0
        limit_pct = _limit_ratio_for_board(code) * 100.0
        if pct + 1e-9 < limit_pct * 0.99:
            continue
        name = ""
...
def _expected_limitup_paths(history_dir: str, yyyymmdd: str) -> List[str]:
    try:
        ds = _to_dashed_date(yyyymmdd)
    except ValueError:
        return []
    paths = [
        os.path.join(history_dir, f"涨停板数据_{ds}.csv"),
        os.path.join(history_dir, "涨停日数据", f"{ds}.json"),
        os.path.join(history_dir, f"daily_change_{ds}.xlsx"),
    ]
    try:
        from utils.limit_up_day_path import resolve_limit_up_day_json_path

        jp = resolve_limit_up_day_json_path(ds, history_dir)
...
def _resolve_today_limitup_universe(
    history_dir: str,
    analysis_date_yyyymmdd: str,
    limitup_csv_path: str = "",
) -> Tuple[Dict[str, str], str, str, str]:
    """
    解析「全部今日涨停」股票池。
    返回 (code->name, limitup_date_yyyymmdd, source_label, used_path)
    """
    query_date = (analysis_date_yyyymmdd or "").strip() or datetime.now().strftime("%Y%m%d")
    tried: List[str] = []

    # 1) 用户指定文件
    if limitup_csv_path and os.path.isfile(limitup_csv_path):
        tried.append(limitup_csv_path)
        if not _csv_looks_nonempty(limitup_csv_path):
            raise RuntimeError(
                f"指定的涨停文件为空或无效占位（无股票行）: {limitup_csv_path}\n"
                f"请换用非空「涨停板数据_YYYY-MM-DD.csv」，或清除后改用自动匹配。"
            )
        codes = _load_limitup_codes(limitup_csv_path)
        m = re.search(r"(20\d{6})", os.path.basename(limitup_csv_path).replace("-", ""))
        date_from_name = m.group(1) if m else query_date
        if not codes:
            raise RuntimeError(
                f"指定涨停文件未能解析出有效代码: {limitup_csv_path}\n"
                f"（已跳过无效占位如 000000）"
            )
        return codes, date_from_name, f"指定CSV:{os.path.basename(limitup_csv_path)}", limitup_csv_path

    # 2) 精确日期 CSV（跳过空占位）
    csv_path = _find_limitup_csv_for_date(history_dir, query_date)
    exact_csv = os.path.join(history_dir, f"涨停板数据_{_to_dashed_date(query_date)}.csv")
    if os.path.isfile(exact_csv) and not _csv_looks_nonempty(exact_csv):
        tried.append(f"{exact_csv}（存在但为空占位，已忽略）")
    if csv_path:
        tried.append(csv_path)
        codes = _load_limitup_codes(csv_path)
        if codes:
            return codes, query_date, f"涨停CSV:{os.path.basename(csv_path)}", csv_path
        tried.append(f"{csv_path}（无有效代码）")

    # 3) 涨停日 JSON
    json_codes, json_path = _load_limitup_codes_from_day_json(history_dir, query_date)
    if json_path:
        tried.append(json_path if json_codes else f"{json_path}（无涨停股票）")
    if json_codes:
        return json_codes, query_date, f"涨停日JSON:{os.path.basename(json_path)}", json_path

    # 4) 本地日涨幅表回退
    dc_codes, dc_path = _load_limitup_codes_from_daily_change(history_dir, query_date)
    if dc_path:
        tried.append(dc_path if dc_codes else f"{dc_path}（未筛出涨停）")
    if dc_codes:
        return dc_codes, query_date, f"daily_change近似涨停:{os.path.basename(dc_path)}", dc_path

    expected = _expected_limitup_paths(history_dir, query_date)
    expected_txt = "\n".join(f"  - {p}" for p in expected)
    tried_txt = "\n".join(f"  - {t}" for t in tried) if tried else "  （无）"
    raise RuntimeError(
        f"分析日期 { _to_dashed_date(query_date) } 无有效涨停股票代码。\n"
        f"期望文件（按优先级）：\n{expected_txt}\n"
        f"已尝试：\n{tried_txt}\n"
        f"请先导出当日涨停板 CSV，或确认 daily_change_{_to_dashed_date(query_date)}.xlsx 存在。"
    )
...
def _label_order_rating(total_score: int) -> str:
    """旧版加权分→五档标签（仅作 legacy 对照，对外展示已改用 v2 强/中/弱）。"""
    s = int(total_score)
    if s <= 18:
        return "🔴 虚封高危"
    if s <= 45:
...
def _apply_cross_sectional_score_adjustment(rows: List[dict]) -> None:
    ...
    for i, row in enumerate(rows):
        ...
        # 旧加权五档仅内部对照；对外「封单评级」固定为 v2：强 / 中 / 弱
        ...
        if _rate_seal_v2 is not None:
            ...
            row["order_rating"] = v2["rating_v2"]  # 强 / 中 / 弱
            row["rating_score"] = int(v2["rating_v2_score"])  # 3 / 2 / 1
            row["confidence_tag"] = {3: "高", 2: "中", 1: "低"}.get(
                int(v2["rating_v2_score"]), "低"
            )
            row["rating_v2_reason"] = v2.get("rating_v2_reason") or ""
        else:
            # 导入失败时降级：仍尽量落到三档名称，避免导出旧五档文案
...
class TodayLimitUpFinderThread(QThread):
    ...
    def run(self):
        try:
            ...
            limitup_path = ""
            limitup_date = query_date
            limitup_code_to_name: Dict[str, str] = {}
            source_label = ""

            if self.source_mode == "today_limitup":
                try:
                    limitup_code_to_name, limitup_date, source_label, limitup_path = _resolve_today_limitup_universe(
                        self.history_dir,
                        query_date,
                        self.limitup_csv_path,
                    )
                except RuntimeError as e:
                    self.error_occurred.emit(str(e))
                    return
            else:
                # 文件/手动：涨停 CSV 仅用于名称与「是否在涨停池」提示，允许回退到最近非空日
                if self.limitup_csv_path and os.path.isfile(self.limitup_csv_path):
                    limitup_path = self.limitup_csv_path
                    m = re.search(r"(20\d{6})", os.path.basename(limitup_path).replace("-", ""))
                    limitup_date = m.group(1) if m else query_date
                    if _csv_looks_nonempty(limitup_path):
                        limitup_code_to_name = _load_limitup_codes(limitup_path)
                        source_label = f"涨停CSV:{os.path.basename(limitup_path)}"
                if not limitup_code_to_name:
                    exact = _find_limitup_csv_for_date(self.history_dir, query_date)
                    if exact:
                        limitup_path, limitup_date = exact, query_date
                        limitup_code_to_name = _load_limitup_codes(exact)
                        source_label = f"涨停CSV:{os.path.basename(exact)}"
                    else:
                        latest_path, latest_date = _find_latest_limitup_file(self.history_dir, query_date)
                        if latest_path:
                            limitup_path, limitup_date = latest_path, latest_date or query_date
                            limitup_code_to_name = _load_limitup_codes(latest_path)
                            source_label = f"涨停CSV:{os.path.basename(latest_path)}"
                        else:
                            try:
                                (
                                    limitup_code_to_name,
                                    limitup_date,
                                    source_label,
                                    limitup_path,
                                ) = _resolve_today_limitup_universe(self.history_dir, query_date, "")
                            except RuntimeError:
                                limitup_code_to_name, limitup_path, source_label = {}, "", "无涨停对照表"
            ...
            if self.source_mode == "today_limitup":
                # 全部今日涨停：以解析到的涨停池为准（剔除 ST），不再与 all_a 求交
                ...
                debug_msg = (
                    f"limitup_source={source_label}; file={os.path.basename(limitup_path) if limitup_path else '-'}; "
                    f"来源=全部今日涨停; 涨停总数={total_limitup}; "
                ...
                debug_msg = (
                    f"limitup_source={source_label or '-'}; "
                    f"来源={'指定文件' if self.source_mode == 'file' else '输入代码'}; "
                    f"待分析={len(matched_codes)}; 其中在涨停池中={in_limitup_cnt}/{len(matched_codes)}"
                )
            ...
            if not matched_codes:
                if n_before_valid > 0:
                    self.error_occurred.emit(
                        f"有效股票代码为空（已剔除无效占位如 000000；剔除前={n_before_valid}）"
                    )
                else:
                    expected = _expected_limitup_paths(self.history_dir, query_date)
                    self.error_occurred.emit(
                        "有效股票代码为空。\n期望路径：\n"
                        + "\n".join(f"  - {p}" for p in expected)
                    )
            ...
            # 第2步：逐只读 tick（本地 cache 优先；大 QMT 可选兜底，不以 miniQMT 为必需）
            ...
            if missing_tick == len(rows) and len(rows) > 0:
                self.debug_info.emit(
                    f"{debug_msg}；警告：全部 {missing_tick} 只均无 tick"
                    f"（本地 data/ticks/{limitup_date} 与 QMT 均未取到）"
                )
...
class LimitUpGeneAnalysisDialog(QDialog):
    ...
    def _streak_table_column_alignments(self) -> List[str]:
        """按表头决定列对齐：封单评级/封单结构、置信度居中。"""
        ...
        headers: List[str] = []
        for c in range(n):
            hi = self.streak_table.horizontalHeaderItem(c)
            headers.append((hi.text() if hi else "").strip())
        # 有「置信度」列 → 按封单结构表处理
        if any(h in ("置信度", "封单评级", "封单结构") for h in headers):
            aligns = [structure_map.get(c, "left") for c in range(n)]
            for c, h in enumerate(headers):
                if h in ("封单评级", "封单结构", "置信度"):
                    aligns[c] = "center"
            return aligns
        return [default_map.get(c, "left") for c in range(n)]

    def _export_streak_excel(self):
        date = getattr(self, "_limitup_date", "") or "export"
        n = self.streak_table.columnCount()
        aligns = self._streak_table_column_alignments()
        ...
        if finish_kind == "list_only":
            ...
            for r_idx, row in enumerate(streak_rows):
                for c_idx, (key, _) in enumerate(structure_headers):
                    ...
                    item = QTableWidgetItem(txt)
                    if key in ("order_rating", "confidence_tag"):

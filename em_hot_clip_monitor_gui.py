...
# -*- coding: utf-8 -*-
"""
东财热门夹档 · 策略表现监控 GUI（独立版）

- 三个图叠画两套回测：空头排列 / 排列并集
- 近窗滚动、开买日票均、累计开买日票均
- 点击某开买日 → 两侧对比摘要 + 票明细
- 图上右键 /「导出当日Excel」→ 导出该开买日（空头+并集同份）
- 回测在子目录；选股文件在父目录
- 「更新选股+回测」：缺口补选股（东财热门-无涨停-MA排列并集-含均线差对照）
  + 买入「开盘夹档+Cond2 clip(2,4)」×2（空头/并集）+ 卖出「综合卖出」×2

用法:
  python em_hot_clip_monitor_gui.py
  python em_hot_clip_monitor_gui.py --auto-run
  python em_hot_clip_monitor_gui.py --window 10 --months 2
"""
...
import em_hot_clip_monitor as zb  # noqa: E402
...
def _n_sel_series_for_asofs(
    ...
    """每个开买日对应的「前一交易日选股只数」（与是否有空头侧成交无关）。"""
    ...
    def run(self):
        try:
            ...
            if not any(reports[v].get("starts") for v in zb.VARIANTS):
                raise RuntimeError(
                    "两套回测均无已了结开买日样本。\n"
                    f"请检查: {zb.DIR}/空头排列 与 {zb.DIR}/排列并集"
                )
...
class PrepareDataWorker(QThread):
    ...
    def run(self):
        try:
            script = ROOT / "tools" / "prepare_em_hot_clip_data.py"
            ...
            self.progress.emit(
                f"开始准备数据（缺口补选股 + 空头/并集双回测，上限 {self.max_days} 天）…"
            )
            env = os.environ.copy()
...
class CompareChartWidget(QWidget):
    ...
    def render(self, reports: Dict[str, dict], highlight: Optional[str] = None):
        ...
        # --- 1 近窗滚动 ---
        ax1.plot(x, ys(zb.VARIANT_BEAR, "roll_mean"), color=C_TB, lw=1.3, marker="o", ms=3, label="空头·近窗日均%")
        ax1.plot(x, ys(zb.VARIANT_UNION, "roll_mean"), color=C_NO, lw=1.3, marker="s", ms=3, label="并集·近窗日均%")
        ax1.axhline(0, color="#999", lw=0.8, ls="--")
        ax1b = ax1.twinx()
        ax1b.plot(x, ys(zb.VARIANT_BEAR, "roll_win"), color=C_TB_WIN, lw=1.2, ls="--", marker=".", ms=3, label="空头·日胜率%")
        ax1b.plot(x, ys(zb.VARIANT_UNION, "roll_win"), color=C_NO_WIN, lw=1.2, ls="--", marker=".", ms=3, label="并集·日胜率%")
        ax1b.set_ylabel("日胜率%")
        ax1b.set_ylim(0, 100)
        ...
        ax1.set_ylabel("近窗日均%")
        ax1.set_title("近窗滚动（开买日票均）· 空头排列 vs 排列并集")
        ax1.grid(True, alpha=0.25)

        # --- 2 开买日票均 + 成交笔数 ---
        ax2.plot(x, ys(zb.VARIANT_BEAR, "day_mean"), color=C_TB, lw=1.3, marker="o", ms=3, zorder=3, label="空头·票均%")
        ax2.plot(x, ys(zb.VARIANT_UNION, "day_mean"), color=C_NO, lw=1.3, marker="s", ms=3, zorder=3, label="并集·票均%")
        ax2.axhline(0, color="#999", lw=0.8, ls="--")
        ax2.set_ylabel("票均%")
        ax2b = ax2.twinx()
        w = 0.26
        n_tb = np.array(ys(zb.VARIANT_BEAR, "n"), dtype=float)
        n_no = np.array(ys(zb.VARIANT_UNION, "n"), dtype=float)
        # 前一日选股共用：勿只取空头 day_view——空头无成交日会缺 n_sel，
        # nan_to_num 后紫柱变成 0，而同日并集仍有成交。
        ...
        # --- 3 累计 ---
        ax3.plot(x, ys(zb.VARIANT_BEAR, "cum_mean"), color=C_TB, lw=1.3, marker="o", ms=3, label="空头·累计")
        ax3.plot(x, ys(zb.VARIANT_UNION, "cum_mean"), color=C_NO, lw=1.3, marker="s", ms=3, label="并集·累计")
        ax3.fill_between(x, ys(zb.VARIANT_BEAR, "cum_mean"), 0, alpha=0.08, color=C_TB)
        ax3.fill_between(x, ys(zb.VARIANT_UNION, "cum_mean"), 0, alpha=0.08, color=C_NO)
...
class EmHotClipMonitorDialog(QDialog):
    def __init__(
        self,
        parent=None,
        auto_run: bool = False,
        ...
        super().__init__(parent)
        self.setWindowTitle("蚂蚁量化 - 东财热门夹档监控")
        self.setWindowFlags(
            self.windowFlags()
        ...
        self.prepare_btn.setToolTip(
            ...
            f"选股=东财热门-无涨停-MA排列并集-含均线差对照；买入=开盘夹档+Cond2 clip(2,4)；卖出=综合卖出；\n"
            f"空头/并集各跑一遍（上限 {self._prepare_max_days} 天）。"
        )
    ...
    def export_asof(self, asof_s: str):
        ...
        default_name = f"东财热门夹档_{asof_s}.xlsx"
    ...
    def show_asof(self, asof_s: str):
        ...
        lines.append("口径：横轴=开买日；蓝=空头排列、橙=排列并集；累计=开买日票均简单加总。")
        self.detail.setPlainText("\n".join(lines))


def main(argv=None) -> int:
    ap = argparse.ArgumentParser(description="东财热门夹档策略监控")

            ...
            if row['is_limit_up']:
                ...
                if prev_bid_vol is not None:
                    ...
                    if bid_vol_diff < 0:
                        ...
                        if cancel_amount > 1000000 and bid_vol_ratio < -0.5:
...
def analyze_multiple_stocks(stock_codes, date):
    ...
    if all_periods:
        ...
        # 检查文件是否已存在
        filename = 'limit_up_analysis_excluedtraded_notbroken.csv'
        if os.path.exists(filename):
            # 如果文件存在，读取现有数据并追加新数据
            existing_df = pd.read_csv(filename, encoding='gbk')
            df_limit_up = pd.concat([existing_df, df_limit_up], ignore_index=True)
        
        # 保存所有数据
        df_limit_up.to_csv(filename, index=False, encoding='gbk')
        print(f"\n涨停开板结果已保存到 {filename}")

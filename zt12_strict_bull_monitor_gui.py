...
# -*- coding: utf-8 -*-
"""
涨停后1–2天严多头 · 策略表现监控 GUI（独立版）

- 三个图叠画两套回测：真突破 / 不看真突破
- 近窗滚动、开买日票均、累计开买日票均
- 点击某开买日 → 两侧对比摘要 + 票明细
- 图上右键 /「导出当日Excel」→ 导出该开买日（真突破+不看真突破同份）
- 回测在子目录；选股文件在父目录
- 「更新选股+回测」：缺口补选股（涨停后的第1到2天-严多头）
  + 买入「突破5日线买入」×2（真突破/不看）+ 卖出「综合卖出」×2

用法:
  python zt12_strict_bull_monitor_gui.py
  python zt12_strict_bull_monitor_gui.py --auto-run
  python zt12_strict_bull_monitor_gui.py --window 10 --months 2
"""
...
import zt12_strict_bull_monitor as zb  # noqa: E402
...
def _sel_count_map(reports: Dict[str, dict]) -> Dict[date, int]:
    """选股日 → 入选只数（两套报告共用同一选股文件）。"""
...
def _n_sel_series_for_asofs(
    asofs: List[date], reports: Dict[str, dict]
) -> np.ndarray:
    """每个开买日对应的「前一交易日选股只数」（与是否有真突破成交无关）。"""
    sel_map = _sel_count_map(reports)
...
class MonitorWorker(QThread):
    ...
    def run(self):
        try:
            ...
            for v in zb.VARIANTS:
                self.progress.emit(f"加载「{v}」按票回测…")
                rep = zb.build_report(
                    window=self.window, months=self.months, variant=v
                )
                reports[v] = _serialize_report(rep)
            if not any(reports[v].get("starts") for v in zb.VARIANTS):
                raise RuntimeError(
                    "两套回测均无已了结开买日样本。\n"
                    f"请检查: {zb.DIR}/真突破 与 {zb.DIR}/不看真突破"
                )
...
class PrepareDataWorker(QThread):
    ...
    def run(self):
        try:
            script = ROOT / "tools" / "prepare_zt12_strict_bull_data.py"
            ...
            self.progress.emit(
                f"开始准备数据（缺口补选股 + 真突破/不看真突破双回测，上限 {self.max_days} 天）…"
            )
            env = os.environ.copy()
...
class CompareChartWidget(QWidget):
    ...
    def render(self, reports: Dict[str, dict], highlight: Optional[str] = None):
        ...
        # --- 1 近窗滚动 ---
        ax1.plot(x, ys(zb.VARIANT_TRUE_BREAKTHROUGH, "roll_mean"), color=C_TB, lw=1.3, marker="o", ms=3, label="真突破·近窗日均%")
        ax1.plot(x, ys(zb.VARIANT_NO_TRUE_BREAKTHROUGH, "roll_mean"), color=C_NO, lw=1.3, marker="s", ms=3, label="不看真突破·近窗日均%")
        ax1.axhline(0, color="#999", lw=0.8, ls="--")
        ax1b = ax1.twinx()
        ax1b.plot(x, ys(zb.VARIANT_TRUE_BREAKTHROUGH, "roll_win"), color=C_TB_WIN, lw=1.2, ls="--", marker=".", ms=3, label="真突破·日胜率%")
        ax1b.plot(x, ys(zb.VARIANT_NO_TRUE_BREAKTHROUGH, "roll_win"), color=C_NO_WIN, lw=1.2, ls="--", marker=".", ms=3, label="不看·日胜率%")
        ax1b.set_ylabel("日胜率%")
        ax1b.set_ylim(0, 100)
        ...
        ax1.set_ylabel("近窗日均%")
        ax1.set_title("近窗滚动（开买日票均）· 真突破 vs 不看真突破")
        ax1.grid(True, alpha=0.25)

        # --- 2 开买日票均 + 成交笔数 ---
        ax2.plot(x, ys(zb.VARIANT_TRUE_BREAKTHROUGH, "day_mean"), color=C_TB, lw=1.3, marker="o", ms=3, zorder=3, label="真突破·票均%")
        ax2.plot(x, ys(zb.VARIANT_NO_TRUE_BREAKTHROUGH, "day_mean"), color=C_NO, lw=1.3, marker="s", ms=3, zorder=3, label="不看真突破·票均%")
        ax2.axhline(0, color="#999", lw=0.8, ls="--")
        ax2.set_ylabel("票均%")
        ax2b = ax2.twinx()
        w = 0.26
        n_tb = np.array(ys(zb.VARIANT_TRUE_BREAKTHROUGH, "n"), dtype=float)
        n_no = np.array(ys(zb.VARIANT_NO_TRUE_BREAKTHROUGH, "n"), dtype=float)
        # 前一日选股共用：勿只取真突破 day_view——真突破无成交日会缺 n_sel，
        # nan_to_num 后紫柱变成 0，而同日不看真突破仍有成交。
        n_sel = _n_sel_series_for_asofs(asofs, reports)
        # 量柱：前一日选股（共用）+ 两侧成交笔数并排
        ax2b.bar(
            x - w,
            np.nan_to_num(n_sel),
            width=w,
        ...
        ax2b.plot(
            x,
            ys(zb.VARIANT_TRUE_BREAKTHROUGH, "win_rate"),
            color=C_TB_WIN,
            lw=1.2,
            ls="--",
            marker=".",
        ...
        ax2b.plot(
            x,
            ys(zb.VARIANT_NO_TRUE_BREAKTHROUGH, "win_rate"),
            color=C_NO_WIN,
            lw=1.2,
            ls="--",
            marker=".",
        ...
        try:
            cnt_hi = float(np.nanmax(np.concatenate([n_tb, n_no, n_sel]))) if len(x) else 0.0
        ...
        ax2.set_title("开买日票均 · 成交笔数 · 票胜率（叠画）")
        ax2.grid(True, alpha=0.25)

        # --- 3 累计 ---
        ax3.plot(x, ys(zb.VARIANT_TRUE_BREAKTHROUGH, "cum_mean"), color=C_TB, lw=1.3, marker="o", ms=3, label="真突破·累计")
        ax3.plot(x, ys(zb.VARIANT_NO_TRUE_BREAKTHROUGH, "cum_mean"), color=C_NO, lw=1.3, marker="s", ms=3, label="不看真突破·累计")
        ax3.fill_between(x, ys(zb.VARIANT_TRUE_BREAKTHROUGH, "cum_mean"), 0, alpha=0.08, color=C_TB)
        ax3.fill_between(x, ys(zb.VARIANT_NO_TRUE_BREAKTHROUGH, "cum_mean"), 0, alpha=0.08, color=C_NO)
...
class Zt12StrictBullMonitorDialog(QDialog):
    def __init__(
        self,
        parent=None,
        auto_run: bool = False,
        ...
        super().__init__(parent)
        self.setWindowTitle("蚂蚁量化 - 涨停后1–2天严多头 · 真突破 vs 不看真突破")
        self.setWindowFlags(
            self.windowFlags()
        ...
        self.prepare_btn.setToolTip(
            ...
            f"选股=涨停后的第1到2天-严多头；买入=突破5日线买入；卖出=综合卖出；\n"
            f"真突破/不看真突破各跑一遍（上限 {self._prepare_max_days} 天）。"
        )
        ...
        self.export_btn.setToolTip(
            "导出当前开买日：前一日选股、两套回测的买卖/收益明细、近窗对比（也可在图上右键）"
        )
    ...
    def export_asof(self, asof_s: str):
        ...
        default_name = f"严多头策略监控_{asof_s}.xlsx"
    ...
    def show_asof(self, asof_s: str):
        ...
        try:
            ...
            sel_map = _sel_count_map(self._reports)
            n_sel = sel_map.get(prev) if prev is not None else None
            if n_sel is None:
                arr = _n_sel_series_for_asofs([asof_d], self._reports)
                n_sel = int(arr[0]) if len(arr) and arr[0] == arr[0] else None
            lines.append(
                f"前一日选股: {prev or '—'}  只数 {n_sel if n_sel is not None else '—'}"
            )
        except Exception:
            pass
        win = None
        ...
        for v in zb.VARIANTS:
            ...
            lines.append("代码   名称       选股日       收益%     清仓日      涨停后第N日  备注")
            lines.append("-" * 78)
            ...
            if not trades:
                ...
                for t in trades:
                    lu = t.get("lu_day")
                    lu_s = "" if lu is None else str(int(lu))
                    lines.append(
                        f"{t.get('code',''):<6} {(t.get('name') or '')[:8]:<8} "
                        f"{t.get('sel',''):<12} {_fmt_pct(t.get('ret')):>8}  "
                        f"{t.get('known_on',''):<10} {lu_s:>6}  {t.get('note','')}"
                    )
        ...
        lines.append("口径：横轴=开买日；蓝=真突破、橙=不看真突破；累计=开买日票均简单加总。")
        self.detail.setPlainText("\n".join(lines))


def main(argv=None) -> int:
    ap = argparse.ArgumentParser(description="涨停后1–2天严多头策略监控")
    ...
    dlg = Zt12StrictBullMonitorDialog(

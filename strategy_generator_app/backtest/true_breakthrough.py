# -*- coding: utf-8 -*-
"""
与 intelligentbuy.py 中「真突破」判定对齐，供回测 tick 撮合使用（无 PyQt / xtdata 依赖）。

真突破 = ①突破量/均量>1.25 AND (②委卖/委买(取小)<0.6 OR ③量/被吃卖档合计>2)
"""
...
# 条件①窗口模式：突破 tick 与前 N 帧（约 30s）内 最大单笔、窗口均量 二者取大（同 per-tick 量纲）
...
# 成交明细 CSV / 选股收益汇总表 共用的真突破字段名（顺序固定）
...
def infer_tick_vol_to_shares_multiplier(raw_df) -> float:
    """与 intelligentbuy：根据 amount/volume/lastPrice 推断量为股或手。"""
    ...
    try:
        ...
        # 向量化差分，避免 iterrows
        amt = probe["amount"].to_numpy(dtype=float, copy=False)
        vol = probe["volume"].to_numpy(dtype=float, copy=False)
        px = probe["lastPrice"].to_numpy(dtype=float, copy=False)
        if len(amt) < 2:
            return 100.0
        d_amt = amt[1:] - amt[:-1]
        d_vol = vol[1:] - vol[:-1]
        px1 = px[1:]
        ok = (d_amt > 0) & (d_vol > 0) & (px1 > 0)
        if not ok.any():
            return 100.0
        ratios = (d_amt[ok] / d_vol[ok]) / px1[ok]
...
def min_cond2_depth_ask_over_bid_ratio_for_rows(
    ...
    """本 tick、近3tick均、近5tick均 三者委卖/委买比取最小。"""
    if not rows:
        return None, 0
...
def choose_incremental_volume_column(rows_list: List[Any]) -> Optional[str]:
    """与 intelligentbuy._per_tick_trade_volumes：多数 tick 有值的增量列名。"""
    inc_names = (
        "lastVol",
        "tradeVol",
        "tradeVolume",
...
def normalize_true_breakthrough_cond1_mode(mode: Any) -> str:
    """条件①算法：tick3=本+前2帧取大；window=本+前10帧内最大单笔/窗口均量取大。"""
...
def max_cond1_breakthrough_volume_window_sh(
    ...
    """
    条件①突破量（窗口）：本 tick 与前 lookback_prior 帧内，
    最大单笔 与 窗口均量 取大（二者均为 per-tick 量纲，可与会话前缀均量比较）。
    不用窗口总量：总量与单笔/均量量纲不同，参与 max 会恒为总量。
    """
    if not per_vols or idx < 0 or idx >= len(per_vols):
        return None
...
def is_breakthrough_break_below_trigger_tick(
    ...
    """
    仅在价格由 >= 触发价 跌破至 < 触发价 的首 tick 返回 True。
    与 is_breakthrough_buy_price_cross_tick 对称，用于「先跌破再突破」类突破买入。
    """
    lp = float(last_price or 0)
    trig = float(trigger_price or 0)
    if lp <= 0 or trig <= 0:
...
def true_breakthrough_export_fields(metrics: Dict[str, Any]) -> Dict[str, str]:
    """将真突破 metrics 转为成交明细/汇总可携带的扁平字段（仅数值与是否通过）。"""
...
def evaluate_true_breakthrough_tick_with_detail(
    ...
    """判定真突破，并返回三条条件的数值明细文本及 metrics（供回测导出）。"""
...
def evaluate_true_breakthrough_tick(
    ...
    """
    当前 tick 是否满足「真突破」三条（与 intelligentbuy 一致）。
    break_row / prev_row 为 tick 行字典；recent_rows 为含当前 tick 在内最近若干 tick。
    """

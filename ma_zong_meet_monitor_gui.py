...
用法:
  python ma_zong_meet_monitor_gui.py
  python ma_zong_meet_monitor_gui.py --auto-run
  python ma_zong_meet_monitor_gui.py --window 10 --months 2
"""
...
class PrepareDataWorker(QThread):
    ...
    def run(self):
        try:
            ...
            self.progress.emit(
                f"开始准备数据（强制重选 + ma10/sell_half 回测，上限 {self.max_days} 天）…"
            )
            env = os.environ.copy()
            ...
            if skipped:
                ...
                self.finished_ok.emit(
                    "选股+回测完成（马总选股逻辑-次日MA10），可点「刷新」看图"
                )
        except Exception as e:
            self.failed.emit(str(e))
...
class MaZongMeetMonitorDialog(QDialog):
    def __init__(
        self,
        parent=None,
        auto_run: bool = False,
        ...
        super().__init__(parent)
        self.setWindowTitle("蚂蚁量化 - 马总满足条件对比监控")
        self.setWindowFlags(
            self.windowFlags()
        ...
        self.prepare_btn.setToolTip(
            ...
            f"选股=马总选股逻辑-次日MA10；买入=收盘上MA10；卖出=sell_half；\n"
            f"持有最多8个交易日强清，无1455破MA20；上限 {self._prepare_max_days} 天。"
        )
        ...
        self.export_btn.setToolTip(
            "导出当前开买日：前一日选股（满足/不满足）、开买收益明细、近窗对比（也可在图上右键）"
        )
    ...
    def export_asof(self, asof_s: str):
        ...
        default_name = f"马总满足条件对比_{asof_s}.xlsx"

...
import trim_export_png as _trim_png_mod
...
def export_inst_tables_one_sheet(tables: Dict[str, pd.DataFrame], out_fp: str) -> None:
    ...
    for title, df in tables.items():
        ...
        for i, h in enumerate(headers):
            ws.cell(row=start_row + 1, column=start_col + i, value=h)
            ws.cell(row=start_row + 1, column=start_col + i).font = title_font
        ...
        block_end_row = max(start_row + 1, start_row + 1 + len(exp))
        for r in range(start_row + 1, block_end_row + 1):
...
def build_longhubang_reports() -> dict:
    ...
    def _top_inst_day(df_mix: pd.DataFrame, side: str, topn: int) -> pd.DataFrame:
        if side == "buy":
            tmp = df_mix[df_mix["净额当日万"] > 0].sort_values("净额当日万", ascending=False).head(topn).copy()
            tmp = tmp.rename(columns={"净额当日万": "机构净买当日万"})
            out_cols = ["code", "name", "买方次数", "卖方次数", "机构净买当日万", "买入三日万", "卖出三日万", "净额三日万"]
        else:
            tmp = df_mix[df_mix["净额当日万"] < 0].sort_values("净额当日万", ascending=True).head(topn).copy()
            tmp["机构净卖当日万"] = tmp["净额当日万"].abs().round(2)
            out_cols = ["code", "name", "买方次数", "卖方次数", "机构净卖当日万", "买入三日万", "卖出三日万", "净额三日万"]
        out = tmp[out_cols].reset_index(drop=True)
        out = out.rename(columns={"code": "股票代码", "name": "股票名称"})
        out.index += 1
        return out

    def _top_inst_3d(df3: pd.DataFrame, side: str, topn: int) -> pd.DataFrame:
        if side == "buy":
            tmp = df3[df3["净额三日万"] > 0].sort_values("净额三日万", ascending=False).head(topn).copy()
            tmp = tmp.rename(columns={"净额三日万": "机构净买三日万"})
            out_cols = ["code", "name", "买方次数", "卖方次数", "机构净买三日万", "买入三日万", "卖出三日万"]
        else:
            tmp = df3[df3["净额三日万"] < 0].sort_values("净额三日万", ascending=True).head(topn).copy()
            tmp["机构净卖三日万"] = tmp["净额三日万"].abs().round(2)
            out_cols = ["code", "name", "买方次数", "卖方次数", "机构净卖三日万", "买入三日万", "卖出三日万"]
        out = tmp[out_cols].reset_index(drop=True)
        out = out.rename(columns={"code": "股票代码", "name": "股票名称"})
        out.index += 1
        return out
    ...
    jg_tables = {
        "机构当日净买 TOP10": _top_inst_day(jg_mix, "buy", 10),
        "机构三日净买 TOP10": _top_inst_3d(jg_3d, "buy", 10),
        "机构当日净卖 TOP10": _top_inst_day(jg_mix, "sell", 10),
        "机构三日净卖 TOP10": _top_inst_3d(jg_3d, "sell", 10),
    }
    merged_title = "机构四榜连排"

...
# 实盘机需要：资金流 / 东财板块 / 选股 / F10 标签（涨停抓取在 main 里单独跑，两边都保留）
LIVE_JOBS: List[JobStep] = [
...
# 全量盘后：在 LIVE 之后追加研究/复盘 GUI
FULL_EXTRA_JOBS: List[JobStep] = [
...
def _parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="交易日盘后自动批跑入口")
    p.add_argument(
        "--live-only",
        action="store_true",
...
def _mode_tag(live_only: bool) -> str:
    return "仅实盘" if live_only else "全量"


def _mode_title_prefix(live_only: bool) -> str:
    return f"[{_mode_tag(live_only)}]"


def _mode_body_line(live_only: bool) -> str:
    if live_only:
        return "**模式**：仅实盘（--live-only）"
    return "**模式**：全量盘后批跑"
...
def _alert_step_fail(
    ...
    *,
    ...
    title = f"{_mode_title_prefix(live_only)} 盘后批跑失败 {day_s} · {step}"
    lines = [
        _mode_body_line(live_only),
        f"**日期**：{day_s}",
        f"**步骤**：{step}",
...
def _notify_daily_batch(
    ...
    *,
    ...
    title = (
        f"{_mode_title_prefix(live_only)} 盘后批跑"
        f"{'成功' if ok else '异常'} {day_s}"
    )
    lines = [
        _mode_body_line(live_only),
        f"**日期**：{day_s}",
    ...
    if live_only:
        lines.append("**COS / 公众号**：已跳过（仅实盘模式）")
    else:
...
def _record_fail(
    ...
    *,
    ...
    if alert:
        _alert_step_fail(
            day, step=step, detail=detail or label, live_only=live_only
        )
    ...
    *,
    ...
    while True:
        ...
        if rc == 0:
            ...
            if attempt > 1:
                _send_server_chan(
                    f"{_mode_title_prefix(live_only)} 公众号草稿已恢复 {today_str}",
                    f"{_mode_body_line(live_only)}\n"
                    f"**日期**：{today_str}\n**结果**：{status}\n"
                    f"**时间**：{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
                )
            return 0, status
...
def _select_jobs(live_only: bool) -> List[JobStep]:
    """后续程序列表：实盘精简 = LIVE；全量 = LIVE + 研究 GUI（F10 仍在 LIVE 末尾，与旧顺序一致需重排）。

    旧全量顺序：资金流 → 东财 → 选股 → 研究GUI… → F10。
    实盘：资金流 → 东财 → 选股 → F10。
    全量：资金流 → 东财 → 选股 → 研究GUI… → F10（与改造前一致）。
    """
    if live_only:
        return list(LIVE_JOBS)
    # FULL: LIVE 前三项 + EXTRA + F10（LIVE 最后一项）
    head = LIVE_JOBS[:-1]
    f10 = LIVE_JOBS[-1]
    return list(head) + list(FULL_EXTRA_JOBS) + [f10]
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    mode_s = _mode_tag(live_only)
    print(f"[执行] {today} 是交易日，开始盘后批跑。模式={mode_s}")
    if live_only:
        print("[模式] 仅实盘：跳过研究 GUI / 盘后量能 / COS / 公众号 / 行情描绘；归档时保留选股结果")
    failed: List[str] = []
    cos_status: Optional[str] = None
    wechat_draft_status: Optional[str] = None
    ...
    # 3) 其余程序（顺序）— 失败步骤名用于报警标题
    jobs = _select_jobs(live_only)
    print(
        f"[执行] 共 {len(jobs)} 个后续程序（涨停抓取与 LU 排名导出已单独执行；模式={mode_s}）。"
    )
    ...
    if not live_only:
        ...
        wx_rc, wechat_draft_status = _upload_wechat_draft_with_retry(
            script_dir, today, today_str, live_only=live_only
        )
        if wx_rc != 0:
            failed.append(
                _job_label("tools/upload_cos_wechat_draft.py", ("--date", today_str))
            )
        ...
        regime_rc = _run_job(
            script_dir, "tools/export_market_regime_to_csv.py", regime_args
        )
        if regime_rc != 0:
            _record_fail(
                failed,
        ...
        print("[跳过] 仅实盘模式：盘后量能 / COS / 公众号 / 行情描绘 均未执行")
    ...
    if failed:
        print(f"[完成] 盘后批跑结束（{mode_s}），存在失败项 {len(failed)} 个。")
        return 1

    print(f"[完成] 盘后批跑结束（{mode_s}），全部成功。")
    return 0

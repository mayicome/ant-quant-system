...
# docstring removed for QMT gbk loader
    ...
    def evaluate_price_map(self, stocks: Any) -> List[Dict[str, Any]]:
        """�� results.stocks �����д�����tick �ص�ͣ��ʱ�Ķ��ף���"""
        events: List[Dict[str, Any]] = []
    ...
    def _process_row(self, code: str, row: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for task in active:
            ...
            if crossed:
                # δҪ����ͻ�ƣ��ϴ�/վ�ϼ�����ͼ�� require_tb=False һ�£�
                require_tb = True
    ...
    def _format_tick_time(raw: Any) -> str:
        ...
        - '14:13:30'
        - '20240620141330' / '20240620 141330'����ð�ţ�
        - unix ��(10λ) / ����(13λ)������ QMT ȫ��ֻ�� time ���� timetag��
        """
        if raw is None:
            return ""
        s = str(raw).strip()
        ...
        if digits_only:
            try:
                n = float(s)
                abs_n = abs(n)
                # 10 λ�� / 13 λ���룻14+ λ�� YYYYMMDDHHMMSS��������
    ...
    def _tick_row_timetag(row: Dict[str, Any]) -> Any:
        """���ȹٷ� timetag��ֱ���ص���ֻ�� time/stime��ȱ�򶵵����� last_tick_time ��ס��"""

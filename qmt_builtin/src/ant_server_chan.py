# -*- coding: utf-8 -*-
"""Server酱推送（大 QMT / 主程序共用思路）。

默认从「蚂蚁量化信息采集系统」库按名称取 SendKey（如 马毅），
也可在 data/notify_server_chan.json 里写 send_key 覆盖。
兼容 Python 3.6（urllib，无 requests 依赖）。
"""
import json
import os
import sqlite3
...
_LAST_ALERT_TS = {}  # type: Dict[str, float]
...
def _config_path():
    # type: () -> str
    return os.path.join(_data_dir(), "notify_server_chan.json")
...
def _lookup_key_from_db(db_path, name):
...
def resolve_send_key(cfg=None):
    # type: (Optional[Dict[str, Any]]) -> Tuple[str, str]
    """返回 (send_key, source_desc)。"""
...
def send_server_chan(title, content, send_key=None):
    ...
    try:
        ...
        # py3.6 Request 无 method=；POST 由 data 推断
...
def notify_alert(title, content, alert_key="default", cooldown_sec=None):
    # type: (str, str, str, Optional[float]) -> Dict[str, Any]
    """带冷却的告警推送，避免同一类告警刷屏。"""
